...
class Active extends Common
{
            ...
            foreach ($this->actives as $active => $data) {
                if (is_array($data) && isset($data['username']) && $data['username'] == $this->username) {
                    ...
                    if (file_exists($root.$data['path'])) {
                        ...
                        $active_list[] = ['path' => $data['path'], 'focused' => $focused];
                    } else {
    ...
    public function Add()
    {
        ...
        if ($process_add) {
            $this->actives[] = ['username' => $this->username, 'path' => $this->path];
            saveJSON('active.php', $this->actives);
            echo formatJSEND('success');
        }
    }
    ...
    public function Rename()
    {
        ...
        foreach ($this->actives as $active => $data) {
            if (is_array($data) && isset($data['username'])) {
                $revised_actives[] = ['username' => $data['username'], 'path' => str_replace($this->path, $this->new_path, $data['path'])];
            }
        }
