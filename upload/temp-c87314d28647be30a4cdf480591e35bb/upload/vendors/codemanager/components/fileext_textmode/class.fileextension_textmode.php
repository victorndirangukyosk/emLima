...
class fileextension_textmode
{
    ...
    private function processFileExtTextModeForm()
    {
        if (!Common::checkAccess()) {
            return ['status' => 'error', 'msg' => 'You are not allowed to edit the file extensions.'];
        }
        ...
        if (!isset($_POST['extension']) || !is_array($_POST['extension'])
            ...
            return json_encode(['status' => 'error', 'msg' => 'incorrect data send']);
        }
        ...
        foreach ($_POST['extension'] as $key => $extension) {
            ...
            if (!$this->validateExtension($extension)) {
                return json_encode(['status' => 'error', 'msg' => 'incorrect extension:'.htmlentities($extension)]);
            }

            if (!$this->validTextMode($textMode)) {
                return json_encode(['status' => 'error', 'msg' => 'incorrect text mode:'.htmlentities($textMode)]);
            }
        ...
        if ('' != $warning) {
            return json_encode(['status' => 'warning', 'msg' => $warning, 'extensions' => $exMap]);
        } else {
            return json_encode(['status' => 'success', 'msg' => 'File extensions are saved successfully.', 'extensions' => $exMap]);
        }
    }
    ...
    private function prcessGetFileExtTextModes()
    {
        ...
        return json_encode(['status' => 'success', 'extensions' => $availEx, 'textModes' => $this->availiableTextModes]);
    }
