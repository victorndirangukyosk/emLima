...
(function(global, $){
    ...
    codiad.filemanager = {
		...
		openInBrowserExt: function(path) {
            $.get(this.controller + '?action=open_in_browser_ext&path=' + path, function(data) {
                var openIBResponse = codiad.jsend.parse(data);
                if (openIBResponse != 'error') {
                    window.open(openIBResponse.url, '_newtab');
                }
		...
		downloadext: function(path) {
            var type = this.getType(path);
            $('#download')
                .attr('src', 'components/filemanager/downloadext.php?path=' + path + '&type=' + type);
        }
    };
