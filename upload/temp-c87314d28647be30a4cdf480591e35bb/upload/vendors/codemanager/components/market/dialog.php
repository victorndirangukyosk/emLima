        ...
        case 'list':
            ...
            <table width="100%">
                <tr>
                    ...
                    <th valign="middle" width="30%" style="white-space:nowrap;"><input style="margin:0;display:inline" onkeyup="codiad.market.search(event, this.value,'<?php echo $_GET['note']; ?>')" value="<?php if (isset($_GET['query'])) {
                    echo $_GET['query'];
                } ?>" placeholder="<?php i18n('Press Enter to Search'); ?>"></th>
                </tr>
             </table>
            ...
            <?php
                ...
                foreach ($market->remote as $data) {
                    ...
                    if (!array_key_exists($data['remote'], $marketplace)) {
                        $marketplace[$data['remote']] = [];
                    }
                    if (!array_key_exists($data['type'], $marketplace[$data['remote']])) {
                        $marketplace[$data['remote']][$data['type']] = [];
                    }
                    if (!array_key_exists($data['category'], $marketplace[$data['remote']][$data['type']])) {
                        $marketplace[$data['remote']][$data['type']][$data['category']] = [];
                    }
                    array_push($marketplace[$data['remote']][$data['type']][$data['category']], $data);
                }
                ksort($marketplace);

                $extLoaded = (extension_loaded('zip') && extension_loaded('openssl') && 1 == ini_get('allow_url_fopen'));
                ...
                foreach ($marketplace as $remote => $markettype) {
                    ...
                    foreach ($markettype as $type => $data) {
                        ksort($data);
                        if ('undefined' == $_GET['type'] || $_GET['type'] == $type) {
                            foreach ($data as $category => $subdata) {
                                usort($subdata, 'sort_name');
                                foreach ($subdata as $addon) {
                                    if (isset($_GET['query']) && (false === strpos(strtolower(trim($addon['name'])), strtolower(trim($_GET['query']))))) {
                                        break;
                                    }
            ...
            <table width="100%">
                <tr>
                    <th valign="middle" align="center" width="40px"><?php
                      if ('undefined' != $_GET['note'] && 'true' == $_GET['note']) {
