...
<script type="text/javascript">
	...
	$( document ).ready(function() {
		...
		if($('#pesapal_phone_number').val().length >= 9) {
	...
	$('#pesapal_phone_number').on('input', function() { 
	...
	$('#button-confirm,#button-retry').on('click', function() {
	    ...
	    if($('#pesapal_phone_number').val().length >= 9) {
	    	$.ajax({
		        type: 'post',
		        url: 'index.php?path=payment/pesapal/confirm',
		        data: 'mobile=' + encodeURIComponent($('#pesapal_phone_number').val()),
	            dataType: 'json',
		        cache: false,
		        beforeSend: function() {
		            $(".overlayed").show();
		        ...
		        success: function(json) {
		        	...
		        	if(json['processed']) {
		        		...
		        		//$('#success_msg').html('A payment request has been sent to the pesapal number '+$('#pesapal_phone_number').val()+'. Please wait for a few seconds then check for your phone for an PESAPAL PIN entry prompt.');

		        		$('#success_msg').html('A payment request has been sent on your above number. Please make the payment by entering pesapal PIN and click on Confirm Payment button after receiving sms from pesapal');
		        		...
		        		console.log('json pesapal1');
		        		$('#button-confirm').hide();
		        		$('#button-retry').hide();
		        		console.log('json pesapal2');

		        	} else {
		        		console.log('json pesapal err');
	...
	$('#button-complete').on('click', function() {
    	...
    	$.ajax({
	        type: 'post',
	        url: 'index.php?path=payment/pesapal/complete',
            dataType: 'json',
	        cache: false,
	        beforeSend: function() {
	            $(".overlayed").show();
