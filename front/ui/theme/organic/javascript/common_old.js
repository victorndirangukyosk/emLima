...
$(document).delegate('.add-cart-btn', 'click', function() {
    ...
    $('.inc-dec-quantity.unique'+$product_id+'-0').css('display','block');
    ...
    /*$.ajax({
        ...
        success: function(json) {
            ...
            if (json['status']) {
                ...
                $("#proceed_to_checkout").attr("href", "index.php?path=checkout/checkout");
...
$(document).on("click", "#add-in-list-button", function () {
    console.log("add-in-list-button js");
     //$('#list-create-form').submit();
    $('#list-message-error').html('');
    $('#list-message-success').html('');
    console.log($('#add-in-list').serialize());

    if($("input:checkbox[name='add_to_list[]']:checked").length > 0 ) {
