    ...
    public function index() {
        ...
        foreach ($order_stores as $os) {
            ...
            if ($this->cart->getTotalProductsByStore($os) && $this->config->get('config_active_store_minimum_order_amount') > $this->cart->getSubTotal()) {
                ...
                $data['min_order_amount_away'] = $this->config->get('config_active_store_minimum_order_amount') - $this->cart->getSubTotal() . ' away from minimum order value.';
        ...
        for ($slide = 1; $slide <= $noofslides; ++$slide) {
            ...
            if (1 == $slide) {
                ...
                for ($i; $i < $itemsperslide; ++$i) {
                    ...
                    if ($result[$i]['variations'][0]['discount_percentage'] > 0) {
                        $output .= '<div><del>' . $result[$i]['variations'][0]['special'] . '</del>(' . $result[$i]['variations'][0]['discount_percentage'] . ')% OFF</div>';
                    }
                    if ($result[$i]['variations'][0]['discount_percentage'] > 0) {
                    $output .= '     <p class="item-price"><span>' . $result[$i]['variations'][0]['discount_price'] . ' / Per' . $result[$i]['variations'][0]['unit'] . '  </span></p>';
                    }
                    if ($result[$i]['variations'][0]['discount_percentage'] <= 0) {
