	...
	class ControllerPavblogSeo extends Controller {
		...
		public function rewrite($link) {
			...
			foreach ($data as $key => $value) {
				if (isset($data['path'])) {
					if (($data['path'] == 'product/product' && $key == 'product_id') || (($data['path'] == 'product/manufacturer/info' || $data['path'] == 'product/product') && $key == 'manufacturer_id') || ($data['path'] == 'information/information' && $key == 'information_id')) {
			...
			if ( ( preg_match( "#pavblog#", $data['path'] ))  && isset($data['id']) ) { 
			  ...
			  else if( $data['path'] == 'pavblog/blogs' ){ 
				$blogConfig = $this->config->get('pavblog');
				$seo = isset($blogConfig['keyword_listing_blogs_page'])?trim($blogConfig['keyword_listing_blogs_page']):"blogs"; 
				$url .= '/'.$seo;
			}
