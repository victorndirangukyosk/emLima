...
class ControllerApiCustomerSettings extends Controller {
    ...
    public function getAdminSettings() {
        ...
        if( true) {
            ...
            foreach ($data as $dat) {
                ...
                if($dat['key'] == 'config_telephone_code' || $dat['key'] == 'config_telephone') {
                    $newData[] = $dat;
                }

                if($dat['key'] == 'config_reward_enabled') {
                    $newData[] = $dat;
                }

                if($dat['key'] == 'reward_status') {
                    $newData[] = $dat;
                } 

                if($dat['key'] == 'config_android_app_link') {
                    $newData[] = $dat;
                }

                if($dat['key'] == 'config_apple_app_link') {
                    $newData[] = $dat;
                } 
                ...
                if($dat['key'] == 'config_update_android') {
                    $newData[] = $dat;
                }

                if($dat['key'] == 'config_update_ios') {
                    $newData[] = $dat;
                }
            ...
            if($this->config->get('iPay_payment_software_mode') == 'test') {
    ...
    public function addDeviceIdToCustomer($args = array())
    {
