...
class ControllerApiCustomerLogin extends Controller {
    ...
    public function addLoginAsCustomer() {
        $log = new Log('error.log');
        $this->load->language('api/login');
        $this->load->language('account/login');
        ...
        if (isset($this->request->post['email'])) {
            //login by email
            $api_info = $this->model_account_api->customerLoginByEmail($this->request->post['email']);
        } elseif (isset($this->request->post['telephone']) && is_numeric($this->request->post['telephone'])) {
            $api_info = $this->model_account_api->customerLoginByTelephone($this->request->post['telephone']);
        }

        if (isset($api_info) && count($api_info) > 0 && $api_info['status']) {
            $tokenId = base64_encode(mcrypt_create_iv(32));
            $issuedAt = time();
            $notBefore = $issuedAt;  //Adding 10 seconds
            $expire = $notBefore + 15780000; // Adding 60 seconds
            ...
            if ($api_info['not_verified']) {
                ...
                if (isset($this->request->post['telephone']) && is_numeric($this->request->post['telephone'])) {
                    //login by phone number
                    $json['error'] = $this->language->get('error_login_mobile');
                } else {
                    $json['error'] = $this->language->get('error_login');
