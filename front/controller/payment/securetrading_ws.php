...
class ControllerPaymentSecureTradingWs extends Controller
{
    ...
    public function process()
    {
        ...
        if ($order_info) {
            ...
            if ($address_info) {
                $order_info['address'] = $address_info['address'].', '.$address_info['city'];
            ...
            if ($this->config->get('securetrading_ws_3d_secure')) {
                ...
                $billing_node->addChild('premise', $order_info['address']);
                $billing_node->addChild('postcode', '');

                $name_node = $billing_node->addChild('name');
                $name_node->addChild('first', $order_info['firstname']);
                $name_node->addChild('last', $order_info['lastname']);
                ...
                if (false !== $response) {
                    ...
                    if ('THREEDQUERY' == $response_xml->response['type']) {
                ...
                $billing_node = $request_node->addChild('billing');
                $billing_node->addChild('premise', $order_info['address']);
                $billing_node->addChild('street', $order_info['address']);
                $billing_node->addChild('town', $order_info['city']);
                $billing_node->addChild('county', $$this->config->get('config_country'));
                $billing_node->addChild('country', $this->config->get('config_country_code'));
                $billing_node->addChild('postcode', '');
    ...
    private function processAuthResponse($response, $order_id)
    {
        ...
        if (false !== $response) {
            ...
            if ('AUTH' == $response_xml->response['type']) {
                $error_code = (int) $response_xml->response->error->code;
                ...
                if (0 == $error_code) {
                    ...
                    $json['redirect'] = $server.'checkout-success'; //$this->url->link('checkout/success');
                    $json['status'] = 1;
                } else {
                    $this->model_payment_securetrading_ws->updateOrder($order_id, $this->config->get('securetrading_ws_declined_order_status_id'));
