...
class ControllerPaymentSagepayServer extends Controller
{
    public function index()
    {
        ...
        if ('1' == $this->config->get('sagepay_server_card')) {
    ...
    public function send()
    {
        ...
        if ('live' == $this->config->get('sagepay_server_test')) {
            $url = 'https://live.sagepay.com/gateway/service/vspserver-register.vsp';

            $payment_data['VPSProtocol'] = '3.00';
        } elseif ('test' == $this->config->get('sagepay_server_test')) {
            $url = 'https://test.sagepay.com/gateway/service/vspserver-register.vsp';

            $payment_data['VPSProtocol'] = '3.00';
        } elseif ('sim' == $this->config->get('sagepay_server_test')) {
        ...
        $payment_data['BillingAddress1'] = substr($address, 0, 100);
        ...
        if ($this->cart->hasShipping()) {
            ...
            if ('US' == $order_info['shipping_iso_code_2']) {
        ...
        if (('OK' == substr($response_data['Status'], 0, 2)) || 'AUTHENTICATED' == $response_data['Status'] || 'REGISTERED' == $response_data['Status']) {
            ...
            if ('PAYMENT' == $this->config->get('sagepay_server_transaction')) {
