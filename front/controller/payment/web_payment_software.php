...
class ControllerPaymentWebPaymentSoftware extends Controller
{
    ...
    public function send()
    {
        ...
        $request .= '&TRANS_TYPE='.urlencode('capture' == $this->config->get('web_payment_software_method') ? 'AuthCapture' : 'AuthOnly');
        ...
        $request .= '&CC_NAME='.urlencode($order_info['firstname'].' '.$order_info['lastname']);
        $request .= '&CC_COMPANY=';
        $request .= '&CC_ADDRESS='.urlencode($order_info['address']);
        $request .= '&CC_CITY='.urlencode($order_info['city']);
        $request .= '&CC_STATE='.urlencode($this->config->get('config_state'));
        $request .= '&CC_ZIP=';
        $request .= '&CC_COUNTRY='.urlencode($this->config->get('config_country'));
        ...
        if ('test' == $this->config->get('web_payment_software_mode')) {
            $request .= '&TEST_MODE=1';
        }
        ...
        if ('00' === (string) $xml->response_code) {
            ...
            $json['redirect'] = $server.'checkout-success'; //$this->url->link('checkout/success', '', 'SSL');
        } else {
