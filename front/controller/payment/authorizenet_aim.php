...
class ControllerPaymentAuthorizeNetAim extends Controller
{
    ...
    {
        if ('live' == $this->config->get('authorizenet_aim_server')) {
            $url = 'https://secure.authorize.net/gateway/transact.dll';
        } elseif ('test' == $this->config->get('authorizenet_aim_server')) {
        ...
        $data['x_first_name'] = html_entity_decode($order_info['firstname'], ENT_QUOTES, 'UTF-8');
        $data['x_last_name'] = html_entity_decode($order_info['lastname'], ENT_QUOTES, 'UTF-8');
        $data['x_company'] = '';
        $data['x_address'] = html_entity_decode($order_info['shipping_address'], ENT_QUOTES, 'UTF-8');
        $data['x_city'] = '';
        $data['x_state'] = html_entity_decode($this->config->get('config_state'), ENT_QUOTES, 'UTF-8');
        $data['x_zip'] = '';
        $data['x_country'] = html_entity_decode($this->config->get('config_country'), ENT_QUOTES, 'UTF-8');
        ...
        $data['x_method'] = 'CC';
        $data['x_type'] = ('capture' == $this->config->get('authorizenet_aim_method')) ? 'AUTH_CAPTURE' : 'AUTH_ONLY';
        ...
        if ($order_info['shipping_method']) {
            $data['x_ship_to_first_name'] = html_entity_decode($order_info['shipping_name'], ENT_QUOTES, 'UTF-8');
            $data['x_ship_to_last_name'] = '';
            $data['x_ship_to_company'] = '';
            $data['x_ship_to_address'] = html_entity_decode($order_info['shipping_address'], ENT_QUOTES, 'UTF-8');
            $data['x_ship_to_city'] = '';
            $data['x_ship_to_state'] = html_entity_decode($this->config->get('config_state'), ENT_QUOTES, 'UTF-8');
            $data['x_ship_to_zip'] = '';
            $data['x_ship_to_country'] = html_entity_decode($this->config->get('config_country'), ENT_QUOTES, 'UTF-8');
        } else {
            $data['x_ship_to_first_name'] = html_entity_decode($order_info['firstname'], ENT_QUOTES, 'UTF-8');
            $data['x_ship_to_last_name'] = html_entity_decode($order_info['lastname'], ENT_QUOTES, 'UTF-8');
            $data['x_ship_to_company'] = '';
            $data['x_ship_to_address'] = html_entity_decode($order_info['shipping_address'], ENT_QUOTES, 'UTF-8');
            $data['x_ship_to_city'] = '';
            $data['x_ship_to_state'] = html_entity_decode($this->config->get('config_state'), ENT_QUOTES, 'UTF-8');
        ...
        if ('test' == $this->config->get('authorizenet_aim_mode')) {
