...
class ControllerPaymentAmazonCheckout extends Controller
{
    ...
    {
        if ('sandbox' == $this->config->get('amazon_checkout_mode')) {
            if ('uk' == $this->config->get('amazon_checkout_marketplace')) {
                $amazon_payment_js = 'https://static-eu.payments-amazon.com/cba/js/gb/sandbox/PaymentWidgets.js';
            } elseif ('de' == $this->config->get('amazon_checkout_marketplace')) {
                $amazon_payment_js = 'https://static-eu.payments-amazon.com/cba/js/de/sandbox/PaymentWidgets.js';
            }
        } elseif ('live' == $this->config->get('amazon_checkout_mode')) {
            if ('uk' == $this->config->get('amazon_checkout_marketplace')) {
    ...
    public function processOrder()
    {
        ...
        if (true !== $cba->setPurchaseItems($parameters_items)) {
    ...
    {
        if (isset($this->request->get['token']) && $this->request->get['token'] == $this->config->get('amazon_checkout_cron_job_token') && 1 == $this->config->get('amazon_checkout_status')) {
            $this->load->model('payment/amazon_checkout');
            $this->load->library('cba');
