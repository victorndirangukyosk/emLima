...
class ControllerPaymentPPProIframe extends Controller
{
    public function index()
    {
        ...
        if ('redirect' == $this->config->get('pp_pro_iframe_checkout_method')) {
    ...
    public function create()
    {
        ...
        if (file_exists(DIR_APPLICATION.'view/theme/'.$this->config->get('config_template').'/stylesheet/stylesheet.css')) {
            $data['stylesheet'] = '/front/ui/theme/'.$this->config->get('config_template').'/stylesheet/stylesheet.css';
        } else {
            $data['stylesheet'] = '/front/ui/theme/default/stylesheet/stylesheet.css';
        }
    ...
    public function notify()
    {
        ...
        if ($order_info) {
            ...
            if (curl_errno($curl)) {
                ...
                if ((0 == strcmp($response, 'VERIFIED') || 0 == strcmp($response, 'UNVERIFIED')) && isset($this->request->post['payment_status'])) {
                    ...
                    if (!$order_info['order_status_id']) {
                        $paypal_order_data = [
                            'order_id' => $order_id,
                            'capture_status' => ('sale' == $this->config->get('pp_pro_iframe_transaction_method') ? 'Complete' : 'NotComplete'),
                        ...
                        $paypal_transaction_data = [
                            ...
                            'transaction_entity' => ('sale' == $this->config->get('pp_pro_iframe_transaction_method') ? 'payment' : 'auth'),
    ...
    private function constructButtonData($order_info)
    {
        ...
        if ($this->cart->hasShipping()) {
            $s_data['L_BUTTONVAR4'] = 'first_name='.urlencode($order_info['shipping_name']);
            $s_data['L_BUTTONVAR5'] = 'last_name=';
            $s_data['L_BUTTONVAR6'] = 'address1='.urlencode($order_info['shipping_address']);
            $s_data['L_BUTTONVAR7'] = 'address2=';
            $s_data['L_BUTTONVAR8'] = 'city=';
        } else {
            $s_data['L_BUTTONVAR4'] = 'first_name='.urlencode($order_info['firstname']);
            $s_data['L_BUTTONVAR5'] = 'last_name='.urlencode($order_info['lastname']);
            $s_data['L_BUTTONVAR6'] = 'address1='.urlencode($order_info['shipping_address']);
            $s_data['L_BUTTONVAR7'] = '';
            $s_data['L_BUTTONVAR8'] = '';
        }

        $s_data['L_BUTTONVAR9'] = 'state='.urlencode($this->config->get('config_state_code'));
        $s_data['L_BUTTONVAR10'] = 'zip=';
        $s_data['L_BUTTONVAR11'] = 'country='.urlencode($this->config->get('config_country_code'));

        $s_data['L_BUTTONVAR12'] = 'billing_first_name='.urlencode($order_info['firstname']);
        $s_data['L_BUTTONVAR13'] = 'billing_last_name='.urlencode($order_info['lastname']);
        $s_data['L_BUTTONVAR14'] = 'billing_address1='.urlencode($order_info['shipping_address']);
        $s_data['L_BUTTONVAR15'] = 'billing_address2=';
        $s_data['L_BUTTONVAR16'] = 'billing_city=';

        $s_data['L_BUTTONVAR17'] = 'billing_state='.urlencode($this->config->get('config_state_code'));
        $s_data['L_BUTTONVAR18'] = 'billing_zip=';
        $s_data['L_BUTTONVAR19'] = 'billing_country='.urlencode($this->config->get('config_country_code'));
        ...
        $s_data['L_BUTTONVAR56'] = 'return='.HTTP_SERVER.'checkout-success';
        ...
        curl_setopt($curl, CURLOPT_HTTPHEADER, ['X-VPS-REQUEST-ID: '.md5($order_info['order_id'].mt_rand())]);
