...
class ControllerPaymentPPExpress extends Controller
{
    ...
    public function express()
    {
        ...
        if (!isset($result['TOKEN'])) {
             ...
             */
            if (1 == $this->config->get('pp_express_debug')) {
        ...
        if (1 == $this->config->get('pp_express_test')) {
    ...
    public function expressReturn()
    {
        ...
        if (true == $this->session->data['paypal']['guest']) {
            ...
            if ($this->cart->hasShipping()) {
                ...
                */
                $country_info = $this->model_payment_pp_express->getCountry($result['PAYMENTREQUEST_0_SHIPTOCOUNTRYCODE']);
                ...
                $zone_info = $this->model_payment_pp_express->getZone($returned_shipping_zone, $returned_shipping_zone, $country_info['country_id']);
            ...
            if ($this->cart->hasShipping()) {
                ...
                foreach ($addresses as $address) {
                    if (trim(strtolower($address['address'])) == trim(strtolower($result['PAYMENTREQUEST_0_SHIPTOSTREET'])) && trim(strtolower($address['postcode'])) == trim(strtolower($result['PAYMENTREQUEST_0_SHIPTOZIP']))) {
                        $match = true;
                ...
                if (false == $match) {
                    ...
                    /*$city_query = $this->db->query( 'select * from `'.DB_PREFIX.'city` WHERE name="'.$result['PAYMENTREQUEST_0_SHIPTOCITY'].'"' );*/

                    $country_info = $this->model_payment_pp_express->getCountry($result['PAYMENTREQUEST_0_SHIPTOCOUNTRYCODE']);

                    $zone_info = $this->model_payment_pp_express->getZoneByName($result['PAYMENTREQUEST_0_SHIPTOSTATE'], $country_info['country_id']);

                    $city_query = $this->model_payment_pp_express->getCity($result['PAYMENTREQUEST_0_SHIPTOCITY']);

                    if ($city_query->num_rows) {
                        $city_id = $city_query->row['city_id'];
                    } else {
                        /*$this->db->query( 'insert into `'.DB_PREFIX.'city` SET name="'.$result['PAYMENTREQUEST_0_SHIPTOCITY'].'"' );

                        $city_id = $this->db->getLastId();*/

                        $city_id = $this->model_payment_pp_express->saveCity($result['PAYMENTREQUEST_0_SHIPTOCITY']);
                    }

                    $address_data = [
                        'name' => $shipping_first_name.' '.$shipping_last_name,
                        'contact_no' => '',
                        'address' => $result['PAYMENTREQUEST_0_SHIPTOSTREET'].(isset($result['PAYMENTREQUEST_0_SHIPTOSTREET2']) ? $result['PAYMENTREQUEST_0_SHIPTOSTREET2'] : ''),
                        'city_id' => $city_id,
                    ];
    ...
    public function expressComplete()
    {
        ...
        if ('' == $redirect) {
            ...
            if ('Success' == $result['ACK']) {
                ...
                $paypal_order_data = [
                    'order_id' => $order_id,
                    'capture_status' => ('Sale' == $this->config->get('pp_express_method') ? 'Complete' : 'NotComplete'),
                ...
                $paypal_transaction_data = [
                    ...
                    'transaction_entity' => ('Sale' == $this->config->get('pp_express_method') ? 'payment' : 'auth'),
                ...
                if (isset($result['REDIRECTREQUIRED']) && true == $result['REDIRECTREQUIRED']) {
                ...
                if ('10486' == $result['L_ERRORCODE0']) {
                    if (isset($this->session->data['paypal_redirect_count'])) {
                        if (2 == $this->session->data['paypal_redirect_count']) {
    ...
    public function ipn()
    {
        ...
        if (!$response) {
            $this->model_payment_pp_express->log(['error' => curl_error($curl), 'error_no' => curl_errno($curl)], 'Curl failed');
        }

        $this->model_payment_pp_express->log(['request' => $request, 'response' => $response], 'IPN data');

        if ('VERIFIED' == (string) $response) {
            if (1 == $this->config->get('pp_express_debug')) {
            ...
            if ($transaction) {
                ...
                if ($transaction['payment_status'] != $this->request->post['payment_status']) {
                    ...
                    $this->model_payment_pp_express->updatePaypalOrderTransaction($this->request->post['payment_status'], $transaction['transaction_id']);
                } elseif ('Pending' == $transaction['payment_status'] && ($transaction['pending_reason'] != $this->request->post['pending_reason'])) {
                    ...
                    $this->model_payment_pp_express->updatePaypalOrderTransactionReason($this->request->post['pending_reason'], $transaction['transaction_id']);
                }
            } else {
                if (1 == $this->config->get('pp_express_debug')) {
                ...
                if ($parent_transaction) {
                     ...
                     */
                    if (isset($this->request->post['payment_status']) && 'Refunded' == $this->request->post['payment_status']) {
                        if (($this->request->post['mc_gross'] * -1) == $parent_transaction['amount']) {
                            $this->model_payment_pp_express->updatePaypalOrderTransactionRefund('Refunded', $parent_transaction['transaction_id']);
                        ...
                        /*$this->db->query( "UPDATE `" . DB_PREFIX . "paypal_order_transaction` SET `payment_status` = 'Refunded' WHERE `transaction_id` = '" . $this->db->escape( $parent_transaction['transaction_id'] ) . "' LIMIT 1" );*/
                            ...
                            $this->model_payment_pp_express->updatePaypalOrderTransactionRefund('Partially-Refunded', $parent_transaction['transaction_id']);
                     ...
                     */
                    if (isset($this->request->post['auth_status']) && 'Completed' == $this->request->post['auth_status'] && 'Pending' == $parent_transaction['payment_status']) {
            ...
            if (isset($this->request->post['txn_type'])) {
                //payment
                if ('recurring_payment' == $this->request->post['txn_type']) {
                    $recurring = $this->model_account_recurring->getProfileByRef($this->request->post['recurring_payment_id']);

                    if (false != $recurring) {
                        $this->model_payment_pp_express->addOrderRecurringTransaction($recurring['order_recurring_id'], $this->request->post['amount']);
...
*/
                        ...
                        if (1 != $recurring['status']) {
                            $this->model_payment_pp_express->updateOrderRecurring($recurring['order_recurring_id']);
                ...
                //suspend
                if ('recurring_payment_suspended' == $this->request->post['txn_type']) {
                    $recurring = $this->model_account_recurring->getProfileByRef($this->request->post['recurring_payment_id']);

                    if (false != $recurring) {
                        $this->model_payment_pp_express->bulkUpdateOrderRecurring($recurring['order_recurring_id'], 6);
                ...
                if ('recurring_payment_suspended_due_to_max_failed_payment' == $this->request->post['txn_type']) {
                    $recurring = $this->model_account_recurring->getProfileByRef($this->request->post['recurring_payment_id']);

                    if (false != $recurring) {
                        $this->model_payment_pp_express->bulkUpdateOrderRecurring($recurring['order_recurring_id'], 7);
                ...
                //payment failed
                if ('recurring_payment_failed' == $this->request->post['txn_type']) {
                    $recurring = $this->model_account_recurring->getProfileByRef($this->request->post['recurring_payment_id']);

                    if (false != $recurring) {
                        $this->model_payment_pp_express->addOrderRecurringTransactionAndType($recurring['order_recurring_id'], 4);
                ...
                if ('recurring_payment_outstanding_payment_failed' == $this->request->post['txn_type']) {
                    $recurring = $this->model_account_recurring->getProfileByRef($this->request->post['recurring_payment_id']);

                    if (false != $recurring) {
                        $this->model_payment_pp_express->addOrderRecurringTransactionAndType($recurring['order_recurring_id'], 8);
                ...
                //outstanding payment
                if ('recurring_payment_outstanding_payment' == $this->request->post['txn_type']) {
                    $recurring = $this->model_account_recurring->getProfileByRef($this->request->post['recurring_payment_id']);
                    ...
                    if (false != $recurring) {
                        ...
                        $this->model_payment_pp_express->addOrderRecurringTransactionAmountAndType($recurring['order_recurring_id'], $this->request->post['amount'], 2);
                        //as there was a payment the recurring is active, ensure it is set to active (may be been suspended before)
                        if (1 != $recurring['status']) {
                            $this->model_payment_pp_express->updateOrderRecurring($recurring['order_recurring_id']);
                ...
                //date_added
                if ('recurring_payment_recurring_date_added' == $this->request->post['txn_type']) {
                    $recurring = $this->model_account_recurring->getProfileByRef($this->request->post['recurring_payment_id']);

                    if (false != $recurring) {
                        $this->model_payment_pp_express->addOrderRecurringTransactionAndType($recurring['order_recurring_id'], 0);
                ...
                //cancelled
                if ('recurring_payment_recurring_cancel' == $this->request->post['txn_type']) {
                    $recurring = $this->model_account_recurring->getProfileByRef($this->request->post['recurring_payment_id']);

                    if (false != $recurring && 3 != $recurring['status']) {
                        ...
                        $this->model_payment_pp_express->addOrderRecurringTransactionAndType($recurring['order_recurring_id'], 5);
...
*/
                        $this->model_payment_pp_express->updateOrderRecurringStatus($recurring['order_recurring_id'], 4);
                    }
                }

                //skipped
                if ('recurring_payment_skipped' == $this->request->post['txn_type']) {
                    $recurring = $this->model_account_recurring->getProfileByRef($this->request->post['recurring_payment_id']);

                    if (false != $recurring) {
                        $this->model_payment_pp_express->addOrderRecurringTransactionAndType($recurring['order_recurring_id'], 3);
                ...
                //expired
                if ('recurring_payment_expired' == $this->request->post['txn_type']) {
                    $recurring = $this->model_account_recurring->getProfileByRef($this->request->post['recurring_payment_id']);

                    if (false != $recurring) {
                        $this->model_payment_pp_express->bulkUpdateOrderRecurringTransaction($recurring['order_recurring_id']);
        ...
        } elseif ('INVALID' == (string) $response) {
    ...
    public function recurringCancel()
    {
        ...
        if ($recurring && !empty($recurring['reference'])) {
            ...
            if (isset($result['PROFILEID'])) {
                $this->model_payment_pp_express->bulkUpdateOrderRecurringTransactionTable($recurring['order_recurring_id']);
    ...
    public function recurringButtons()
    {
        ...
        if (2 == $recurring['status'] || 3 == $recurring['status']) {
            $data['buttons'][] = [
