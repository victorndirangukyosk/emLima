...
class ControllerPaymentPPStandard extends Controller
{
    public function index()
    {
        ...
        if ($order_info) {
            ...
            $data['first_name'] = html_entity_decode($order_info['firstname'], ENT_QUOTES, 'UTF-8');
            $data['last_name'] = html_entity_decode($order_info['lastname'], ENT_QUOTES, 'UTF-8');
            $data['address1'] = html_entity_decode($order_info['shipping_address'], ENT_QUOTES, 'UTF-8');
            $data['address2'] = '';
            $data['city'] = '';
            $data['zip'] = '';
            $data['country'] = $this->config->get('config_country_code');
            $data['email'] = $order_info['email'];
            $data['invoice'] = $this->session->data['order_id'].' - '.html_entity_decode($order_info['firstname'], ENT_QUOTES, 'UTF-8').' '.html_entity_decode($order_info['lastname'], ENT_QUOTES, 'UTF-8');
            ...
            $data['return'] = $server.'checkout-success'; //$this->url->link('checkout/success');
