...
class ModelAccountAddress extends Model {
    ...
    public function addAddress($data) {
        ...
        $this->db->query("INSERT INTO " . DB_PREFIX . "address SET customer_id = '" . (int) $this->customer->getId() . "', name = '" . $this->db->escape($data['name']) . "', contact_no = '" . $this->db->escape($data['contact_no']) . "', city_id = '" . $this->db->escape($data['city_id']) . "', address_type = '" . $this->db->escape($data['address_type']) ."', flat_number = '" . $this->db->escape($data['flat_number']) ."', building_name = '" . $this->db->escape($data['building_name']) ."', latitude = '" . $this->db->escape($data['lat']) ."', longitude = '" . $this->db->escape($data['lng'])."', landmark = '" . $this->db->escape($data['landmark']) ."', zipcode = '" . $this->db->escape($data['zipcode']). "', address = '" . $this->db->escape($data['address']) . "'");
    ...
    public function editAddress($address_id, $data) {
        ...
        $this->db->query("UPDATE " . DB_PREFIX . "address SET name = '" . $this->db->escape($data['name']) . "', contact_no = '" . $this->db->escape($data['contact_no']) . "', address_type = '" . $this->db->escape($data['address_type']) ."', flat_number = '" . $this->db->escape($data['flat_number']) ."', building_name = '" . $this->db->escape($data['building_name']) ."', latitude = '" . $this->db->escape($data['lat']) ."', longitude = '" . $this->db->escape($data['lng']) ."', landmark = '" . $this->db->escape($data['landmark']) ."', zipcode = '" . $this->db->escape($data['zipcode']) . "', address = '" . $this->db->escape($data['address']) . "', city_id = '" . $this->db->escape($data['city_id']) . "' WHERE address_id  = '" . (int) $address_id . "' AND customer_id = '" . (int) $this->customer->getId() . "'");
    ...
    public function getCheckoutQuestion($data = array()) {
    ...
    public function addressCheck($address,$zipcode = '') {
        ...
        if(isset($response_a->status) && $response_a->status == 'OK') {
            ...
            foreach ($response_a->results[0]->address_components as $key => $value) {
                ...
                //print_r($value);
                if(strpos($zipcode, $value->long_name) !== false ) {
