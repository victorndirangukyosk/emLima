...
//$log->write($customer_details);
    ...
    public function addPezeshaCustomer($data) {
        $this->db->query('INSERT INTO ' . DB_PREFIX . "pezesha_customers SET customer_id = '" . (int) $data['customer_id'] . "', customer_uuid = '" . $this->db->escape($data['customer_uuid']) . "', pezesha_customer_id = '" . $this->db->escape($data['pezesha_customer_id']) . "', credit_period = '" . $this->db->escape($data['credit_period']) . "', created_at = NOW()");
        $customer_id = $this->db->getLastId();
        return $customer_id;
    }
