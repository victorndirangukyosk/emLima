    ...
    public function addfarmer($data) {
        ...
        if(isset($data['telephone'])) {
            ...
            $data['telephone'] = preg_replace("/[^0-9]/", "", $data['telephone']);
              ...
              $this->db->query("INSERT INTO " . DB_PREFIX . "farmer SET   name = '" . $this->db->escape($data['name']) . "', email_id = '" . $this->db->escape($data['email']) . "', farm_address = '" . $this->db->escape($data['Address']) . "', contact_number = '" . $this->db->escape($data['telephone']) . "', farmer_type = '" . $this->db->escape($data['farmertype']) . "', work_on_farm = '" . $this->db->escape($data['farm']) . "', country = '" .$this->db->escape($data['country_id']). "', village = '" . $this->db->escape($data['town']) . "', business_entity = '" . $this->db->escape($data['businessentity']) . "', name_of_farm = '" .  $this->db->escape($data['nameoffarm']) . "', total = '" . $this->db->escape($data['Total'] ). "', crop_type = '" . $this->db->escape($data['Crop'] ). "', crops_grown = '" . $this->db->escape($data['cropsgrown'] ). "', crop_produce = '" . $this->db->escape($data['cropproduce'] ). "', sell_produce = '" . $this->db->escape($data['sellproduce'] ). "'");
