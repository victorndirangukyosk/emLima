...
class ModelAccountReturn extends Model {
	public function addReturn($data) {
		...
		$this->db->query("INSERT INTO `" . DB_PREFIX . "return` SET product_id = '" . (int)$data['product_id'] . "', order_id = '" . (int)$data['order_id'] . "', customer_id = '" . (int)$this->customer->getId() . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', product = '" . $this->db->escape($data['product']) ."', unit = '" . $this->db->escape($data['unit']) ."', price = '" . $this->db->escape($data['price'])  . "', model = '" . $this->db->escape($data['model']) . "', quantity = '" . (int)$data['quantity'] . "', opened = '" . (int)$data['opened'] . "', return_reason_id = '" . (int)$data['return_reason_id'] . "',  return_action_id = '" . (int)$data['return_action_id'] . "',  return_status_id = '" . (int)$this->config->get('config_return_status_id') . "', comment = '" . $this->db->escape($data['comment']) . "', date_ordered = '" . $this->db->escape($data['date_ordered']) . "', date_added = NOW(), date_modified = NOW()");
	...
	public function updateGetAmount($order_id,$total) {
        ...
        if($order_info && $deliveryAlreadyCreated && $order_info['payment_code'] == 'cod') {
