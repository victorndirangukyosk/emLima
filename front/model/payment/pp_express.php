...
class ModelPaymentPPExpress extends Model {
	...
	public function getCountry($result) {
		
		return $this->db->query( "SELECT * FROM `" . DB_PREFIX . "country` WHERE `iso_code_2` = '" . $this->db->escape( $result) . "' AND `status` = '1' LIMIT 1" )->row;
	}

	public function getZone($returned_shipping_zone,$returned_shipping_zone,$country_id) {
		
		return $this->db->query( "SELECT * FROM `" . DB_PREFIX . "zone` WHERE (`name` = '" . $this->db->escape( $returned_shipping_zone ) . "' OR `code` = '" . $this->db->escape( $returned_shipping_zone ) . "') AND `status` = '1' AND `country_id` = '" . (int)$country_id. "' LIMIT 1" )->row;
	}

	public function getZoneByName($result,$country_id) {
		
		return $this->db->query( "SELECT * FROM `" . DB_PREFIX . "zone` WHERE `name` = '" . $this->db->escape( $result) . "' AND `status` = '1' AND `country_id` = '" . (int)$country_id. "'" )->row;
	}

	public function getCity($result) {
		
		return $this->db->query( 'select * from `'.DB_PREFIX.'city` WHERE name="'.$result.'"' );
	}
	
	public function saveCity($result) {
		
		$this->db->query( 'insert into `'.DB_PREFIX.'city` SET name="'.$result.'"' );

		return $this->db->getLastId();
	}

	public function updatePaypalOrderTransaction($payment_status,$transaction_id) {
		$this->db->query( "UPDATE `" . DB_PREFIX . "paypal_order_transaction` SET `payment_status` = '" . $payment_status. "' WHERE `transaction_id` = '" . $transaction_id) . "' LIMIT 1" );
	}

	public function updatePaypalOrderTransactionReason($pending_reason,$transaction_id) {
		$this->db->query( "UPDATE `" . DB_PREFIX . "paypal_order_transaction` SET `pending_reason` = '" . $pending_reason. "' WHERE `transaction_id` = '" . $this->db->escape( $transaction_id) . "' LIMIT 1" );
	}

	public function updatePaypalOrderTransactionRefund($status,$transaction_id) {
		$this->db->query( "UPDATE `" . DB_PREFIX . "paypal_order_transaction` SET `payment_status` = '".$status."' WHERE `transaction_id` = '" . $this->db->escape( $transaction_id) . "' LIMIT 1" );
	}

	public function addOrderRecurringTransaction($order_recurring_id,$amount) {
	...
	public function updateOrderRecurring($order_recurring_id) {
		$this->db->query( "UPDATE `" . DB_PREFIX . "order_recurring` SET `status` = 2 WHERE `order_recurring_id` = '" . (int)$order_recurring_id. "'" );
	}

	public function bulkUpdateOrderRecurring($order_recurring_id,$type) {
		
		$this->db->query( "INSERT INTO `" . DB_PREFIX . "order_recurring_transaction` SET `order_recurring_id` = '" . (int)$order_recurring_id. "', `date_added` = NOW(), `type` = '".$type."'" );

		$this->db->query( "UPDATE `" . DB_PREFIX . "order_recurring` SET `status` = 3 WHERE `order_recurring_id` = '" . (int)$order_recurring_id. "' LIMIT 1" );
	}
	
	public function addOrderRecurringTransactionAndType($order_recurring_id,$type) {
		$this->db->query( "INSERT INTO `" . DB_PREFIX . "order_recurring_transaction` SET `order_recurring_id` = '" . (int)$order_recurring_id. "', `date_added` = NOW(), `type` = '".$type."'" );
	}

	public function addOrderRecurringTransactionAmountAndType($order_recurring_id,$amount,$type) {
		$this->db->query( "INSERT INTO `" . DB_PREFIX . "order_recurring_transaction` SET `order_recurring_id` = '" . (int)$order_recurring_id. "', `date_added` = NOW(), `amount` = '" . (float)$amount. "', `type` = '".$type."'" );
	}

	public function updateOrderRecurringStatus($order_recurring_id,$status) {
		$this->db->query( "UPDATE `" . DB_PREFIX . "order_recurring` SET `status` = '".$status."' WHERE `order_recurring_id` = '" . (int)$order_recurring_id. "' LIMIT 1" );
	}

	public function bulkUpdateOrderRecurringTransaction($order_recurring_id) {
		$this->db->query( "INSERT INTO `" . DB_PREFIX . "order_recurring_transaction` SET `order_recurring_id` = '" . (int)$order_recurring_id. "', `date_added` = NOW(), `type` = '9'" );

		$this->db->query( "UPDATE `" . DB_PREFIX . "order_recurring` SET `status` = 5 WHERE `order_recurring_id` = '" . (int)$order_recurring_id. "' LIMIT 1" );
	}

	public function bulkUpdateOrderRecurringTransactionTable($order_recurring_id) {
