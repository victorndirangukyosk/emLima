...
class ModelPaymentIugu extends Model
{
    ...
    public function getMethod($total)
    {
        ...
        if ($this->config->get('iugu_total') > 0 && $this->config->get('iugu_total') > $total) {
            $status = false;
        } else {
            $status = true;
        }
    ...
    public function getTotals()
    {
        ...
        foreach ($total_data as $key => $total) {
            if (('total' != $total['code']) && ('sub_total' != $total['code']) && ('iugu_discount' != $total['code'])) {
                $count = $this->cart->countProducts() + $key;
                $data[$count] = [
                    'description' => $total['title'],
                    'quantity' => 1,
                    'price_cents' => $this->currency->format($total['value'], 'BRL', null, false) * 100,
                ];
            }
        }
    ...
    public function getDiscount($sub_total = null, $payment_method = null)
    {
        ...
        if (!is_null($payment_method) && ('iugu_credit_card' == $payment_method || 'credit_card' == $payment_method)) {
        ...
        if (!is_null($payment_method) && ('iugu_billet' == $payment_method || 'bank_slip' == $payment_method)) {
        ...
        if (!is_null($payment_method) && 'all' == $payment_method) {
                ...
                if ($key <= $installments_total) {
                    if ($installments_total > $installments_interest_free) {
                        ...
                        $installments[$key] = [
                            ...
                            'text_value' => $this->currency->format($installment_value),
                            'total' => $installment_total,
                            'text_total' => $this->currency->format($installment_total),
                        ];
                    } else {
                        $installments[$key] = [
                            'installment' => $key,
                            'value' => $this->getOrderTotal($this->session->data['order_id']),
                            'text' => $this->currency->format($this->getOrderTotal($this->session->data['order_id'])),
                        ];
                    }
                }
            }
