...
class ModelPaymentFirstdataRemote extends Model
{
    ...
    public function capturePayment($data, $order_id)
    {
        ...
        $token = '';
        $payment_token = '';
        if (1 == $this->config->get('firstdata_remote_card_storage')) {
            if (isset($this->request->post['cc_choice']) && 'new' != $this->request->post['cc_choice']) {
                $payment_token = $this->request->post['cc_choice'];
            } elseif (isset($this->request->post['cc_store']) && 1 == $this->request->post['cc_store']) {
        ...
        if ('APPROVED' == strtoupper($response['transaction_result']) && !empty($token)) {
    ...
    {
        if (1 == $this->config->get('firstdata_remote_debug')) {
