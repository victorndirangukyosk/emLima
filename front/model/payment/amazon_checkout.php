...
class ModelPaymentAmazonCheckout extends Model
{
    ...
    public function isAmazonOrder($order_id)
    {
        if ($this->config->get('amazon_checkout_status')) {
            $status = 1 == $this->db->query('SELECT COUNT(*) AS `count` FROM '.DB_PREFIX.'order_amazon WHERE order_id =  '.(int) $order_id)->row['count'];
        } else {
            $status = false;
        }
    ...
    {
        return '1' == $this->db->query('SELECT `free_shipping` FROM `'.DB_PREFIX.'order_amazon` WHERE `order_id` = '.(int) $order_id)->row['free_shipping'];
    }
