...
class Update
{
    ...
    public function Init()
    {
        ...
        if (!file_exists(DATA.'/version.php')) {
            if (file_exists(BASE_PATH.'/.git/HEAD')) {
                ...
                $version[] = ['version' => $local[0]['version'], 'time' => time(), 'optout' => 'true', 'name' => ''];
                saveJSON('version.php', $version);
            } else {
                $remote = $this->getRemoteVersion('install_man');
                $version[] = ['version' => $remote[0]['commit']['sha'], 'time' => time(), 'optout' => 'true', 'name' => ''];
                saveJSON('version.php', $version);
            }
        } else {
            $local = $this->getLocalVersion();
            ...
            if (file_exists(BASE_PATH.'/.git/HEAD')) {
                ...
                if ('' == $local[0]['version'] && $local[0]['name'] == $_SESSION['user']) {
                    ...
                    $version[] = ['version' => $remote[0]['commit']['sha'], 'time' => time(), 'optout' => 'true', 'name' => $_SESSION['user']];
                    saveJSON('version.php', $version);
                }
            }
    ...
    {
        $version[] = ['version' => '', 'time' => time(), 'optout' => 'true', 'name' => $_SESSION['user']];
        saveJSON('version.php', $version);
    }
    ...
    {
        $current = getJSON('version.php');
        $version[] = ['version' => $current[0]['version'], 'time' => $current[0]['time'], 'optout' => 'true', 'name' => $current[0]['name']];
        saveJSON('version.php', $version);
    }
    ...
    public function Check()
    {
        ...
        foreach ($commits as $commit) {
            if ($local[0]['version'] != $commit['sha']) {
                if (false === strpos($commit['commit']['message'], 'Merge')) {
        ...
        return '['.formatJSEND('success', ['currentversion' => $local[0]['version'], 'remoteversion' => $latest, 'message' => $message, 'archive' => $archive, 'nightly' => $nightly, 'name' => $local[0]['name']]).']';
    }
