...
class diff_match_patch
{
    ...
    public function diff_main($text1, $text2, $checklines = true)
    {
        ...
        if ('' !== $commonprefix) {
            array_unshift($diffs, [DIFF_EQUAL, $commonprefix]);
        }
        if ('' !== $commonsuffix) {
            array_push($diffs, [DIFF_EQUAL, $commonsuffix]);
        }
     ...
     * @private
     */
    public function diff_compute($text1, $text2, $checklines)
    {
        if ('' === $text1) {
        ...
        if (false !== $i) {
            ...
            $diffs = [
                [DIFF_INSERT, mb_substr($longtext, 0, $i)],
                [DIFF_EQUAL, $shorttext],
                [DIFF_INSERT, mb_substr($longtext, $i + mb_strlen($shorttext))],
            ];
     ...
     * @private
     */
    public function diff_linesToChars($text1, $text2)
    {
     ...
     * @private
     */
    public function diff_linesToCharsMunge($text, &$lineArray, &$lineHash)
    {
        $chars = '';
     ...
     * @private
     */
    public function diff_charsToLines(&$diffs, $lineArray)
    {
     ...
     * @private
     */
    public function diff_map($text1, $text2)
    {
     ...
     * @private
     */
    public function diff_path1($v_map, $text1, $text2)
    {
        $path = [];
        $x = mb_strlen($text1);
        $y = mb_strlen($text2);
     ...
     * @private
     */
    public function diff_path2($v_map, $text1, $text2)
    {
        $path = [];
        $pathLength = 0;
        $x = mb_strlen($text1);
     ...
     * @private
     */
    public function diff_halfMatchI($longtext, $shorttext, $i)
    {
        ...
        while (false !== ($j = mb_strpos($shorttext, $seed, $j + 1))) {
    ...
    public function diff_cleanupSemantic(&$diffs)
    {
        ...
        $length_changes2 = 0;
        while ($pointer < count($diffs)) {
            if (DIFF_EQUAL == $diffs[$pointer][0]) { // equality found
                ...
                if (null !== $lastequality && (mb_strlen($lastequality) <= $length_changes1) && (mb_strlen($lastequality) <= $length_changes2)) {
                    // Duplicate record
                    $zzz_diffs = array_splice($diffs, $equalities[$equalitiesLength - 1], 0, [[
                        DIFF_DELETE,
                        $lastequality,
                    ]]);
    ...
    public function diff_cleanupSemanticLossless(&$diffs)
    {
        ...
        while ($pointer < count($diffs) - 1) {
            if (DIFF_EQUAL == $diffs[$pointer - 1][0] && DIFF_EQUAL == $diffs[$pointer + 1][0]) {
     ...
     */
    public function diff_cleanupSemanticScore($one, $two)
    {
    ...
    public function diff_cleanupEfficiency(&$diffs)
    {
        ...
        while ($pointer < count($diffs)) {
            if (DIFF_EQUAL == $diffs[$pointer][0]) { // equality found
                ...
                if (DIFF_DELETE == $diffs[$pointer][0]) {
                    $post_del = true;
                } else {
                    $post_ins = true;
                }
                 ...
                 */
                if ($lastequality && (($pre_ins && $pre_del && $post_ins && $post_del) || ((mb_strlen($lastequality) < $this->Diff_EditCost / 2) && 3 == ($pre_ins + $pre_del + $post_ins + $post_del)))) {
                    // Duplicate record
                    $zzz_diffs = array_splice($diffs, $equalities[$equalitiesLength - 1], 0, [[
                        DIFF_DELETE,
                        $lastequality,
    ...
    {
        array_push($diffs, [DIFF_EQUAL, '']); // Add a dummy entry at the end.
        $pointer = 0;
        $count_delete = 0;
        $count_insert = 0;
        $text_delete = '';
                ...
                case DIFF_EQUAL:
                    ...
                    if (0 !== $count_delete || 0 !== $count_insert) {
                        if (0 !== $count_delete && 0 !== $count_insert) {
                            ...
                            if (0 !== $commonlength) {
                                if (($pointer - $count_delete - $count_insert) > 0 && DIFF_EQUAL == $diffs[$pointer - $count_delete - $count_insert - 1][0]) {
                                    $diffs[$pointer - $count_delete - $count_insert - 1][1] .= mb_substr($text_insert, 0, $commonlength);
                                } else {
                                    array_splice($diffs, 0, 0, [[
                                        DIFF_EQUAL,
                    ...
                    } elseif (0 !== $pointer && DIFF_EQUAL == $diffs[$pointer - 1][0]) {
    ...
    public function diff_xIndex($diffs, $loc)
    {
        ...
        $last_chars2 = 0;
        for ($x = 0; $x < count($diffs); ++$x) {
            if (DIFF_INSERT !== $diffs[$x][0]) { // Equality or deletion.
                $chars1 += mb_strlen($diffs[$x][1]);
            }
            if (DIFF_DELETE !== $diffs[$x][0]) { // Equality or insertion.
        ...
        if (count($diffs) != $x && DIFF_DELETE === $diffs[$x][0]) {
            return $last_chars2;
        }
     ...
     * @private
     */
    public function match_bitap($text, $pattern, $loc)
    {
     ...
     * @private
     */
    public function match_bitapScore($e, $x, $pattern, $loc)
    {
     ...
     * @private
     */
    public function match_alphabet($pattern)
    {
        $s = [];
     ...
     * @private
     */
    public function patch_addContext($patch, $text)
    {
        ...
        $i = 0;
        while (
            (0 === mb_strlen($pattern) // Javascript's indexOf/lastIndexOd return 0/strlen respectively if pattern = ''
    ...
    {
        if (is_string($a) && is_string($opt_b) && null === $opt_c) {
        ...
        } elseif (is_array($a) && null === $opt_b && null === $opt_c) {
            // Method 2: diffs
            // Compute text1 from diffs.
            $diffs = $a;
            $text1 = $this->diff_text1($diffs);
        } elseif (is_string($a) && is_array($opt_b) && null === $opt_c) {
        ...
        for ($x = 0; $x < count($diffs); ++$x) {
            ...
            if (!$patchDiffLength && DIFF_EQUAL !== $diff_type) {
            ...
            if (DIFF_INSERT !== $diff_type) {
                $char_count1 += mb_strlen($diff_text);
            }
            if (DIFF_DELETE !== $diff_type) {
        ...
        for ($x = 0; $x < count($patches); ++$x) {
            ...
            if (mb_strlen($text1) > Match_MaxBits) {
                ...
                if (-1 != $start_loc) {
                    ...
                    if (-1 == $end_loc || $start_loc >= $end_loc) {
    ...
    public function patch_addPadding(&$patches)
    {
        ...
        $patch = &$patches[0];
        $diffs = &$patch->diffs;
        if (0 == count($diffs) || DIFF_EQUAL != $diffs[0][0]) {
            // Add nullPadding equality.
            array_unshift($diffs, [DIFF_EQUAL, $nullPadding]);
        ...
        $diffs = &$patch->diffs;
        if (0 == count($diffs) || DIFF_EQUAL != $diffs[count($diffs) - 1][0]) {
            // Add nullPadding equality.
            array_push($diffs, [DIFF_EQUAL, $nullPadding]);
            $patch->length1 += $paddingLength;
            $patch->length2 += $paddingLength;
            ...
            if ($patches[$x]->length1 > Match_MaxBits) {
                ...
                while (0 !== count($bigpatch->diffs)) {
                    ...
                    if ('' !== $precontext) {
                        ...
                        array_push($patch->diffs, [DIFF_EQUAL, $precontext]);
                    }
                    while (0 !== count($bigpatch->diffs) && $patch->length1 < $patch_size - $this->Patch_Margin) {
                        ...
                        if (DIFF_INSERT === $diff_type) {
                            ...
                            $empty = false;
                        } elseif (DIFF_DELETE === $diff_type && 1 == count($patch->diffs) && DIFF_EQUAL == $patch->diffs[0][0] && (mb_strlen($diff_text) > 2 * $patch_size)) {
                            // This is a large deletion.  Let it pass in one chunk.
                            $patch->length1 += mb_strlen($diff_text);
                            $start1 += mb_strlen($diff_text);
                            $empty = false;
                            array_push($patch->diffs, [$diff_type, $diff_text]);
                    ...
                    if ('' !== $postcontext) {
                        ...
                        if (0 !== count($patch->diffs) && DIFF_EQUAL === $patch->diffs[count($patch->diffs) - 1][0]) {
                            $patch->diffs[count($patch->diffs) - 1][1] .= $postcontext;
                        } else {
                            array_push($patch->diffs, [DIFF_EQUAL, $postcontext]);
                        }
                    }
                    if (!$empty) {
    ...
    public function patch_fromText($textline)
    {
        ...
        while ($textPointer < count($text)) {
            ...
            while ($textPointer < count($text)) {
                ...
                if ('-' == $sign) {
                    // Deletion.
                    array_push($patch->diffs, [DIFF_DELETE, $line]);
                } elseif ('+' == $sign) {
                    // Insertion.
                    array_push($patch->diffs, [DIFF_INSERT, $line]);
                } elseif (' ' == $sign) {
                    // Minor equality.
                    array_push($patch->diffs, [DIFF_EQUAL, $line]);
                } elseif ('@' == $sign) {
                    // Start of next patch.
                    break;
                } elseif ('' === $sign) {
