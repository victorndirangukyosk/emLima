...
class CBA
{
    ...
    public function processFeedResponses($settings, $db)
    {
        ...
        for ($i = 0; $i < count($submission_ids); $i += 50) {
            ...
            $response = $this->getMwsResponse('POST', '/', [], $args);
            $xml = simplexml_load_string($response);

            if (isset($xml->Error->Code) && 'RequestThrottled' == (string) $xml->Error->Code) {
                return;
            }
            ...
            if (isset($xml->GetReportListResult) || isset($xml->GetReportListByNextTokenResult)) {
                if (isset($xml->GetReportListResult)) {
                    ...
                    $next_token = ('true' == (string) $xml->GetReportListResult->HasNext) ? (string) $xml->GetReportListResult->NextToken : false;
                } else {
                    $list = $xml->GetReportListByNextTokenResult->ReportInfo;
                    $next_token = ('true' == (string) $xml->GetReportListByNextTokenResult->HasNext) ? (string) $xml->GetReportListByNextTokenResult->NextToken : false;
                }
                ...
                foreach ($list as $list_item) {
                    ...
                    $report = $this->getMwsResponse('POST', '/', [], $args);
                    ...
                    foreach ($lines as $line) {
                        $values = explode("\t", $line);
                        if (isset($values[5]) && ('Error' == $values[4] || 'Fatal' == $values[4])) {
    ...
    public function processOrderReports($settings, $db)
    {
        ...
        while ($xml && (isset($xml->GetReportListResult) || isset($xml->GetReportListByNextTokenResult))) {
            ...
            foreach ($list as $list_item) {
                ...
                $report_response = $this->getMwsResponse('POST', '/', [], $args);
    ...
    public function orderAdjustment($flat_file)
    {
        ...
        $response = $this->getMwsResponse('POST', '/', $args, [], $flat_file, $headers);
    ...
    public function orderCanceled($order)
    {
        ...
        $response = $this->getMwsResponse('POST', '/', $args, [], $flat, $headers);
    ...
    public function orderShipped($order)
    {
        ...
        $response = $this->getMwsResponse('POST', '/', $args, [], $xml, $headers);
    ...
    private function getMwsResponse($http_method, $uri, $get_args, $post_args, $post_body = '', $headers = [])
    {
        if ('uk' == $this->getMarketplace()) {
            $string_to_sign = $http_method."\nmws.amazonservices.co.uk\n".$uri."\n";
        } elseif ('de' == $this->getMarketplace()) {
