...
abstract class BaseFacebook
{
    ...
    protected function _restserver($params)
    {
      ...
      $params
        ...
        if ('auth.expiresession' === $method ||
        'auth.revokeauthorization' === $method) {
            $this->destroySession();
        }
    ...
    {
        if (!$signed_request || false === strpos($signed_request, '.')) {
        ...
        || self::SIGNED_REQUEST_ALGORITHM !== strtoupper($data['algorithm'])
    ) {
            self::errorLog(
    ...
    protected function getCurrentUrl()
    {
        ...
        $host = 'localhost'; //$this->getHttpHost();
    ...
    protected function shouldRetainParam($param)
    {
        foreach (self::$DROP_QUERY_PARAMS as $drop_query_param) {
          ...
          0 === strpos($param, $drop_query_param.'=')) {
                return false;
            }
        }
    ...
    protected function throwAPIException($result)
    {
      ...
      case 'Exception':
        ...
        if ((false !== strpos($message, 'Error validating access token')) ||
            (false !== strpos($message, 'Invalid OAuth access token')) ||
            (false !== strpos($message, 'An active access token must be used'))
        ) {
            $this->destroySession();
        }
        break;
     ...
     *
     * @return bool Returns TRUE if $big matches $small
     */
     ...
     * @param string $small The input string
     *
     * @return bool TRUE if $big ends with $small
     */
