...
class Openid
{
    ...
    public function __construct($host = 'localhost')
    {
        ...
        if (false !== ($host_end = strpos($this->trustRoot, '/', 8))) {
    ...
    public function hostExists($url)
    {
        ...
        return (bool) gethostbynamel($server);
    }
    ...
    protected function request_streams($url, $method = 'GET', $params = [], $update_claimed_id)
    {
        ...
        case 'HEAD':
            ...
            if (405 == intval(substr($headers[0], strlen('HTTP/1.1 ')))) {
        ...
        for ($i = 0; $i < 5; ++$i) {
            if ($yadis) {
                ...
                if (isset($headers['content-type'])
                    && (false !== strpos($headers['content-type'], 'application/xrds+xml')
                        || false !== strpos($headers['content-type'], 'text/xml'))
                ) {
    ...
    protected function getAxAttributes()
    {
        ...
        if (isset($this->data['openid_ns_ax'])
            ...
            foreach ($this->data as $key => $val) {
                if ('openid_ns_' == substr($key, 0, strlen('openid_ns_'))
