...
class PHPExcel_ReferenceHelper
{
    ...
    public function updateCellReference($pCellRange = 'A1', $pBefore = 'A1', $pNumCols = 0, $pNumRows = 0)
    {
        ...
        if (false !== strpos($pCellRange, '!')) {
            return $pCellRange;
        // Is it a range or a single cell?
        } elseif (false === strpos($pCellRange, ':') && false === strpos($pCellRange, ',')) {
            // Single cell
        ...
        foreach ($pPhpExcel->getWorksheetIterator() as $sheet) {
            foreach ($sheet->getCellCollection(false) as $cellID) {
                ...
                if ((null !== $cell) && (PHPExcel_Cell_DataType::TYPE_FORMULA == $cell->getDataType())) {
                    ...
                    if (false !== strpos($formula, $oldName)) {
    ...
    {
        if (false !== strpos($pCellRange, ':') || false !== strpos($pCellRange, ',')) {
            // Update range
    ...
    {
        if (false === strpos($pCellReference, ':') && false === strpos($pCellReference, ',')) {
            // Get coordinates of $pBefore
            list($beforeColumn, $beforeRow) = PHPExcel_Cell::coordinateFromString($pBefore);
