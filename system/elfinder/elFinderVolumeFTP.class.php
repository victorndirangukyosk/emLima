...
class elFinderVolumeFTP extends elFinderVolumeDriver
{
    ...
    protected function parseRaw($raw, $base, $nameOnly = false)
    {
        ...
        $stat['name'] = $name;
        $stat['mime'] = 'd' == substr(strtolower($info[0]), 0, 1) ? 'directory' : $this->mimetype($stat['name']);
    ...
    protected function normalizeRawWindows($raw)
    {
        ...
        $format = 8 === strlen($date) ? 'm-d-yH:iA' : 'Y-m-dH:i';
    ...
    {
        return 0 === strpos($this->mimetype($path), 'text/') ? FTP_ASCII : FTP_BINARY;
    }
    ...
    {
        return $path == $parent || 0 === strpos($path, $parent.$this->separator);
    }
    ...
    protected function _stat($path)
    {
        ...
        if (!$this->MLSTsupprt) {
            ...
            return $this->convEncIn(isset($this->cache[$outPath]) ? $this->cache[$outPath] : []);
        }
        $raw = ftp_raw($this->connect, 'MLST '.$path);
        if (is_array($raw) && count($raw) > 1 && 2 == substr(trim($raw[0]), 0, 1)) {
            ...
            foreach ($parts as $part) {
                ...
                switch ($key) {
                    case 'type':
                        $stat['mime'] = false !== strpos($val, 'dir') ? 'directory' : $this->mimetype($path);
                        break;
    ...
    protected function _subdirs($path)
    {
        foreach (ftp_rawlist($this->connect, $path) as $str) {
            ...
            if ($name && '.' !== $name && '..' !== $name && 'd' === substr(strtolower($info[0]), 0, 1)) {
                return true;
            }
        }
