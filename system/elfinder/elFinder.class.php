...
class elFinder
{
    ...
    protected $commands = [
        ...
        'paste' => ['dst' => true, 'targets' => true, 'cut' => false, 'mimes' => false, 'renames' => false, 'suffix' => false],
        'upload' => ['target' => true, 'FILES' => true, 'mimes' => false, 'html' => false, 'upload' => false, 'name' => false, 'upload_path' => false, 'chunk' => false, 'cid' => false, 'node' => false, 'renames' => false, 'suffix' => false],
        ...
        'dim' => ['target' => true],
        'resize' => ['target' => true, 'width' => true, 'height' => true, 'mode' => false, 'x' => false, 'y' => false, 'degree' => false, 'quality' => false],
    ...
    protected function upload($args)
    {
                    ...
                    if ('data:' === substr($url, 0, 5)) {
                        ...
                        if ($data && ($headers = get_headers($url, true)) && !empty($headers['Content-Disposition'])) {
                            if (preg_match('/filename\*?=(?:(.+?)\'\')?"?([a-z0-9_.~%-]+)"?/i', $headers['Content-Disposition'], $m)) {
                                ...
                                if ($m[1] && 'UTF-8' !== strtoupper($m[1]) && function_exists('mb_convert_encoding')) {
        ...
        foreach ($files['name'] as $i => $name) {
            ...
            if (!$_target || false === ($file = $volume->upload($fp, $_target, $name, $tmpname))) {
    ...
    protected function paste($args)
    {
        ...
        foreach ($targets as $target) {
            ...
            if (false == ($file = $dstVolume->paste($srcVolume, $target, $dst, $cut))) {
    ...
    protected function get($args)
    {
        ...
        if ($args['conv'] && function_exists('mb_detect_encoding') && function_exists('mb_convert_encoding')) {
            ...
            if ($mime && 'text' === strtolower(substr($mime, 0, 4))) {
                if ($enc = mb_detect_encoding($content, mb_detect_order(), true)) {
                    if ('utf-8' !== strtolower($enc)) {
        ...
        if (false === $json || strlen($json) < strlen($content)) {
            if ($args['conv']) {
