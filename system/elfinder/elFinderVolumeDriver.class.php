...
abstract class elFinderVolumeDriver
{
    ...
    public function file($hash)
    {
        ...
        if ($isRoot) {
            $file['uiCmdMap'] = (isset($this->options['uiCmdMap']) && is_array($this->options['uiCmdMap'])) ? $this->options['uiCmdMap'] : [];
    ...
    protected function attr($path, $name, $val = null, $isDir = null)
    {
        ...
        return null === $perm ? (is_null($val) ? $this->defaults[$name] : $val) : (bool) $perm;
    }
    ...
    protected function canCreateTmb($path, $stat, $checkTmbPath = true)
    {
        return (!$checkTmbPath || $this->tmbPathWritable)
            ...
            && ('gd' == $this->imgLib ? 'image/jpeg' == $stat['mime'] || 'image/png' == $stat['mime'] || 'image/gif' == $stat['mime'] : true);
    }
