...
class elFinderVolumeDropbox extends elFinderVolumeDriver
{
    ...
    public function netmountPrepare($options)
    {
        ...
        if ('init' === $options['user']) {
            ...
            if ('init' === $options['pass']) {
                ...
                $_SESSION['elFinderDropboxTokens'] = [$_GET['uid'], $tokens['token'], $tokens['token_secret']];
    ...
    {
        $url = ((isset($_SERVER['HTTPS']) && 'off' !== strtolower($_SERVER['HTTPS'])) ? 'https://' : 'http://')
               .$_SERVER['SERVER_NAME']                                              // host
              .(80 == $_SERVER['SERVER_PORT'] ? '' : ':'.$_SERVER['SERVER_PORT'])  // port
    ...
    private function checkDB()
    {
        ...
        if ($res && isset($_REQUEST['init'])) {
            ...
            if (!$chk || false === strpos(strtoupper($chk[0]), 'UNIQUE')) {
                // remake
                $this->DB->exec('DROP TABLE '.$this->DB_TableName);
                $res = false;
            }
    ...
    public function getContentUrl($hash, $options = [])
    {
        if (false == ($file = $this->file($hash)) || !$file['url'] || 1 == $file['url']) {
            ...
            $url = '';
            if (isset($cache['share']) && false !== strpos($cache['share'], $this->dropbox_dlhost)) {
            ...
            if (!$url) {
                try {
                    ...
                    $url = $res['url'];
                    if (false === strpos($url, 'www.dropbox.com')) {
