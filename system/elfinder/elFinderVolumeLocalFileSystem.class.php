...
class elFinderVolumeLocalFileSystem extends elFinderVolumeDriver
{
    ...
    protected function configure()
    {
        ...
        if ($this->options['tmbPath']) {
            $this->options['tmbPath'] = false === strpos($this->options['tmbPath'], DIRECTORY_SEPARATOR)
        ...
        if ($root['read'] && !$this->tmbURL && $this->URL) {
            if (0 === strpos($this->tmbPath, $this->root)) {
    ...
    protected function _inpath($path, $parent)
    {
        ...
        if ($real_path && $real_parent) {
            return $real_path === $real_parent || 0 === strpos($real_path, rtrim($real_parent, DIRECTORY_SEPARATOR).DIRECTORY_SEPARATOR);
        }
    ...
    protected function _dimensions($path, $mime)
    {
        ...
        return 0 === strpos($mime, 'image') && false !== ($s = @getimagesize($path))
            ? $s[0].'x'.$s[1]
            : false;
    }
    ...
    protected function readlink($path)
    {
        ...
        if (0 !== strpos($target, $this->systemRoot)) {
    ...
    public function localFileSystemSearchIteratorFilter($file, $key, $iterator)
    {
        ...
        return (false === $this->stripos($file->getFilename(), $this->doSearchCurrentQuery)) ? false : true;
    }
} // END class
