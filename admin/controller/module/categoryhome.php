    ...
    public function index()
    {
        ...
        if (('POST' == $this->request->server['REQUEST_METHOD']) && ($this->validate())) {
            if (!isset($this->request->get['module_id'])) {
                $this->model_extension_module->addModule('categoryhome', $this->request->post);
            } else {
        ...
        $data['categories'][] = ['category_id' => 0, 'name' => $this->language->get('entry_root_category')];
        ...
        if (!isset($this->request->get['module_id'])) {
            $data['action'] = $this->url->link('module/categoryhome', 'token='.$this->session->data['token'], 'SSL');
        } else {
            $data['action'] = $this->url->link('module/categoryhome', 'token='.$this->session->data['token'].'&module_id='.$this->request->get['module_id'], 'SSL');
        }
