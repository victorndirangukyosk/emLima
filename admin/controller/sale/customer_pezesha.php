...
class ControllerSaleCustomerPezesha extends Controller {
    ...
    public function applyloanfordeliveredorder() {
        ...
        if ($order_id != NULL && $order_id > 0) {
            ...
            if ($order_info != NULL && $pezesha_order_info != NULL && ($pezesha_order_info['loan_id'] == NULL || $pezesha_order_info['loan_id'] <= 0) && $pezesha_order_info['customer_id'] == $order_info['customer_id'] && $pezesha_order_info['order_id'] == $order_info['order_id']) {
                $log->write('applyloanfordeliveredorder');
    ...
    public function applyloanfordeliveredordernew() {
        $log = new Log('error.log');
        $order_id = isset($this->request->post['order_id']) && $this->request->post['order_id'] > 0 ? $this->request->post['order_id'] : 0;
        ...
        if ($order_id != NULL && $order_id > 0) {
            ...
            if ($order_info != NULL && $pezesha_order_info != NULL && ($pezesha_order_info['loan_id'] == NULL || $pezesha_order_info['loan_id'] <= 0) && $pezesha_order_info['customer_id'] == $order_info['customer_id'] && $pezesha_order_info['order_id'] == $order_info['order_id']) {
                ...
                if (is_array($customer_device_info) && count($customer_device_info) > 0) {
                    $log->write('applyloanfordeliveredorder_2');
                    $response = $this->load->controller('pezesha/pezesha/applyloanfordeliveredordernew', $customer_info);
                }
    ...
    public function applyloanone() {
        ...
        $order_id = isset($this->request->post['order_id']) && $this->request->post['order_id'] > 0 ? $this->request->post['order_id'] : NULL;
        ...
        $response = NULL;
        if (is_array($customer_device_info) && count($customer_device_info) > 0) {
            $response = $this->load->controller('pezesha/pezesha/applyloanone', $customer_info);
        }
