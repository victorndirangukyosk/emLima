...
class ControllerReportCustomerOrderCount extends Controller
{


    public function index()
    {
        $this->load->language('report/customer_order_count');
        ...
        $data['breadcrumbs'][] = [
            ...
            'href' => $this->url->link('report/customer_order_count', 'token='.$this->session->data['token'].$url, 'SSL'),
        ];
        ...
        $pagination->url = $this->url->link('report/customer_order_count', 'token='.$this->session->data['token'].$url.'&page={page}', 'SSL');
        ...
        $this->response->setOutput($this->load->view('report/customer_order_count.tpl', $data));
    }
    ...
    public function order_countexcel()
    {
        $this->load->language('report/customer_order_count');
        ...
        if (is_array($customerresults) && count($customerresults) > 0) {
            ...
            foreach ($customerresults as $result) {               
                ...
                foreach ($months as $month) {
                    ...
                    $data['customers'][$i][$monthname]=round($totalpermonth['TotalOrders'],2);
               }
               $data['customers'][$i]['Total Order Count']= $totalOrders;

                $data['customers'][$i]['Total Order Value']=round($OrdersValue,2);
                if($OrdersValue>0 && $totalOrders>0)
                {
        ...
        $this->model_report_excel->download_customer_order_count_excel($data['customers']);
    }
