...
class ControllerPezeshaPezesha extends Controller {
    ...
    public function userregistration() {
        ...
        $log->write('auth_response');
        $body = array('full_names' => $customer_device_info['firstname'] . ' ' . $customer_device_info['lastname'], 'phone' => 'users', 'other_phone_nos' => $this->config->get('pezesha_client_secret'), 'national_id' => $this->config->get('pezesha_client_id'), 'dob' => $this->config->get('pezesha_merchant_key'), 'email' => '', 'merchant_id' => '', 'merchant_reg_date' => '', 'location' => '', 'geo_location' => '');
        $curl = curl_init();
        if (ENV == 'production') {
