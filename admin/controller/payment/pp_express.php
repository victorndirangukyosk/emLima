...
class ControllerPaymentPPExpress extends Controller
{
    ...
    public function index()
    {
        ...
        $data['text_ipn_url'] = HTTPS_CATALOG.'index.php?path=payment/pp_express/ipn';
            ...
            if ($transaction) {
                ...
                if ($result) {
                    ...
                    if ($parent_transaction['amount'] == abs($transaction['amount'])) {
                        $this->model_payment_pp_express->updatePaypalOrder('Refunded', $this->db->escape($transaction['parent_transaction_id']));
                    } else {
                        $this->model_payment_pp_express->updatePaypalOrder('Partially-Refunded', $this->db->escape($transaction['parent_transaction_id']));
                    }
    ...
    public function capture()
    {
        ...
        if (isset($this->request->post['order_id']) && $this->request->post['amount'] > 0 && isset($this->request->post['order_id']) && isset($this->request->post['complete'])) {
            ...
            if (1 == $this->request->post['complete']) {
                $complete = 'Complete';
            } else {
            ...
            if (false == $result) {
            ...
            } elseif (isset($result['ACK']) && 'Failure' != $result['ACK'] && 'FailureWithWarning' != $result['ACK']) {
                $transaction['transaction_id'] = $result['TRANSACTIONID'];
                $transaction['payment_type'] = $result['PAYMENTTYPE'];
                $transaction['payment_status'] = $result['PAYMENTSTATUS'];
                $transaction['pending_reason'] = (isset($result['PENDINGREASON']) ? $result['PENDINGREASON'] : '');
                ...
                if (1 == $this->request->post['complete'] && $transaction['remaining'] > 0) {
                    $transaction['void'] = [
                        'paypal_order_id' => $paypal_order['paypal_order_id'],
                        'transaction_id' => '',
                        'parent_transaction_id' => $paypal_order['authorization_id'],
    ...
    public function void()
    {
        ...
        if (isset($this->request->post['order_id']) && '' != $this->request->post['order_id']) {
            ...
            if ('Failure' != $result['ACK'] && 'FailureWithWarning' != $result['ACK']) {
                $transaction = [
                    'paypal_order_id' => $paypal_order['paypal_order_id'],
                    'transaction_id' => '',
                    'parent_transaction_id' => $paypal_order['authorization_id'],
    ...
    public function doRefund()
    {
        ...
        if (isset($this->request->post['transaction_id']) && isset($this->request->post['refund_full'])) {
            ...
            if (0 == $this->request->post['refund_full'] && 0 == $this->request->post['amount']) {
                ...
                if ($paypal_order) {
                    ...
                    if (1 == $this->request->post['refund_full']) {
                    ...
                    if (false == $result) {
                    ...
                    } elseif ('Failure' != $result['ACK'] && 'FailureWithWarning' != $result['ACK']) {
                        $transaction['transaction_id'] = $result['REFUNDTRANSACTIONID'];
                        $transaction['payment_type'] = $result['REFUNDSTATUS'];
                        ...
                        if ($result['TOTALREFUNDEDAMOUNT'] == $this->request->post['amount_original']) {
                            $this->model_payment_pp_express->updatePaypalOrder('Refunded', $this->db->escape($this->request->post['transaction_id']));
                        } else {
                            $this->model_payment_pp_express->updatePaypalOrder('Partially-Refunded', $this->db->escape($this->request->post['transaction_id']));
                        }
    ...
    public function doSearch()
    {
        ...
        if (isset($this->request->post['date_start'])) {
            ...
            if ('Failure' != $result['ACK'] && 'FailureWithWarning' != $result['ACK'] && 'Warning' != $result['ACK']) {
    ...
    public function recurringCancel()
    {
        ...
        if ($recurring && !empty($recurring['reference'])) {
            ...
            if (isset($result['PROFILEID'])) {
                $this->model_payment_pp_express->multipleUpdate((int) $recurring['order_recurring_id']);
    ...
    public function recurringButtons()
    {
        ...
        if (2 == $recurring['status_id'] || 3 == $recurring['status_id']) {
            $data['buttons'][] = [
