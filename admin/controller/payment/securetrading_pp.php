...
class ControllerPaymentSecureTradingPp extends Controller
{
    ...
    public function index()
    {
        ...
        if (isset($this->request->post['securetrading_pp_cards_accepted'])) {
            ...
            if (null == $data['securetrading_pp_cards_accepted']) {
                $data['securetrading_pp_cards_accepted'] = [];
            }
        }
    ...
    public function void()
    {
        ...
        if (isset($this->request->post['order_id']) && '' != $this->request->post['order_id']) {
            ...
            if (false !== $void_response) {
                $response_xml = simplexml_load_string($void_response);

                if ('ERROR' == $response_xml->response['type'] || '0' != (string) $response_xml->response->error->code) {
        ...
        if (isset($this->request->post['order_id']) && '' != $this->request->post['order_id'] && isset($amount) && $amount > 0) {
            ...
            if (false !== $release_response) {
                ...
                if ('ERROR' == $response_xml->response['type'] || '0' != (string) $response_xml->response->error->code) {
                    ...
                    if ($total_released >= $securetrading_pp_order['total'] || 100 == $securetrading_pp_order['settle_type']) {
        ...
        if (isset($this->request->post['order_id']) && !empty($this->request->post['order_id'])) {
            ...
            if (false !== $rebate_response) {
                ...
                if ('0' == $error_code) {
                    ...
                    if ($total_released <= 0 && 1 == $securetrading_pp_order['release_status']) {
                        $json['status'] = 1;
