...
class ControllerExtensionModification extends Controller
{
    ...
    public function delete()
    {
        ...
        if (isset($this->request->post['selected']) && $this->validate()) {
            foreach ($this->request->post['selected'] as $modification_id) {
                ...
                } elseif (file_exists(DIR_VQMOD.'xml/'.$modification_id)) {
        ...
        if ($this->validate()) {
            ...
            foreach ($xml as $xml) {
                ...
                foreach ($files as $file) {
                    ...
                    if ('catalog' == substr($file->getAttribute('path'), 0, 7)) {
                        $path = DIR_CATALOG.str_replace('../', '', substr($file->getAttribute('path'), 8));
                    }

                    if ('admin' == substr($file->getAttribute('path'), 0, 5)) {
                        $path = DIR_APPLICATION.str_replace('../', '', substr($file->getAttribute('path'), 6));
                    }

                    if ('system' == substr($file->getAttribute('path'), 0, 6)) {
                        ...
                        if ($files) {
                            foreach ($files as $file) {
                                ...
                                foreach ($operations as $operation) {
                                    ...
                                    if ($ignoreif) {
                                        if ('true' != $ignoreif->getAttribute('regex')) {
                                            if (false !== strpos($modification[$key], $ignoreif->textContent)) {
                                                continue;
                                            }
                                        } else {
                                    ...
                                    // Search and replace
                                    if ('true' != $operation->getElementsByTagName('search')->item(0)->getAttribute('regex')) {
                                        // Search
            ...
            if (file_exists(DIR_VQMOD.'xml')) {
                ...
                $_modCheck = new ShopModification();
    ...
    public function enable()
    {
        ...
        if (isset($this->request->get['modification_id']) && $this->validate()) {
            ...
            } elseif (file_exists(DIR_VQMOD.'xml/'.$this->request->get['modification_id'])) {
        ...
        foreach ($files as $file) {
            ...
            if (!empty($xml)) {
                try {
                    ...
                    $status = ('.xml_' != substr($file, -5)) ? 1 : 0;
                    $file_suffix = ('.xml_' != substr($file, -5)) ? '.xml' : '.xml_';
        ...
        if (file_exists(DIR_VQMOD.'xml/')) {
            ...
            if (!empty($data['vqmods'])) {
                ...
                foreach ($data['modifications'] as $modification) {
                    if ('.xml_' != substr($modification['modification_id'], -5)) {
                        // enable OCMOD
                        ...
                        if (empty($vqmod_add)) {
                            ...
                            foreach ($data['vqmods'] as $vqmod) {
                                if ('.xml_' != substr($vqmod['vqmod_id'], -5)) {
                                    // enable VQMOD
