...
class ControllerExtensionMarketplace extends Controller
{
    ...
    public function index()
    {
        ...
        $this->document->addScript('ui/javascript/marketplace/marketplace.js');
        $this->document->addStyle('ui/stylesheet/marketplace.css');
        ...
        if ($this->validate()) {
            ...
            if ($post) {
                ...
                curl_setopt($curl, CURLOPT_URL, $this->apiBaseUrl.'index.php?path=api/key');
        ...
        if ($this->validate()) {
            if (isset($this->session->data['cookie']) && isset($this->request->get['api'])) {
                ...
                foreach ($this->request->get as $key => $value) {
                    if ('path' != $key && 'token' != $key && 'store_id' != $key && 'api' != $key) {
                        $url_data[$key] = $value;
                    }
                }
                ...
                curl_setopt($curl, CURLOPT_FORBID_REUSE, false);
                curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
                curl_setopt($curl, CURLOPT_URL, $this->apiBaseUrl.'index.php?path='.$this->request->get['api'].($url_data ? '&'.http_build_query($url_data) : ''));
