...
class ControllerMarketingAffiliate extends Controller
{
    ...
    public function add()
    {
        ...
        if (('POST' == $this->request->server['REQUEST_METHOD']) && $this->validateForm()) {
            $affiliate_id = $this->model_marketing_affiliate->addAffiliate($this->request->post);
    ...
    protected function getForm()
    {
        ...
        if (isset($this->request->get['affiliate_id']) && ('POST' != $this->request->server['REQUEST_METHOD'])) {
    ...
    protected function validateForm()
    {
        ...
        } elseif ('bank' == $this->request->post['payment']) {
    ...
    public function commission()
    {
        ...
        if (('POST' == $this->request->server['REQUEST_METHOD']) && $this->user->hasPermission('modify', 'marketing/affiliate')) {
            $this->model_marketing_affiliate->addCommission($this->request->get['affiliate_id'], $this->request->post['description'], $this->request->post['amount']);
        ...
        $results = $this->model_marketing_affiliate->getCommissions($this->request->get['affiliate_id'], ($page - 1) * 10, 10);
        ...
        $data['balance'] = $this->currency->format($this->model_marketing_affiliate->getCommissionTotal($this->request->get['affiliate_id']), $this->config->get('config_currency'));

        $commission_total = $this->model_marketing_affiliate->getTotalCommissions($this->request->get['affiliate_id']);

        $pagination = new Pagination();
        $pagination->total = $commission_total;
        $pagination->page = $page;
        $pagination->limit = $this->config->get('config_limit_admin');
        $pagination->url = $this->url->link('marketing/affiliate/commission', 'token='.$this->session->data['token'].'&affiliate_id='.$this->request->get['affiliate_id'].'&page={page}', 'SSL');
        ...
        $this->response->setOutput($this->load->view('marketing/affiliate_commission.tpl', $data));
    }
