...
class ControllerCommonHeader extends Controller {
    ...
    public function index() {
        ...
        if (!isset($this->request->get['token']) || !isset($this->session->data['token']) || ($this->request->get['token'] != $this->session->data['token'])) {
            ...
            $product_total = $this->model_catalog_vendor_product->getTotalProducts(array('filter_quantity' => 0));

            $data['product_total'] = $product_total;

            $data['product'] = $this->url->link('catalog/vendor_product', 'token=' . $this->session->data['token'] . '&filter_quantity=0', 'SSL');

            $product_low_total = $this->model_catalog_vendor_product->getTotalProducts(array('filter_quantity' => 10));
