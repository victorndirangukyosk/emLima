...
class ControllerSettingStore extends Controller {
    ...
    public function getForm() {
        ...
        foreach ($results as $result) {
            ...
            if ($result['code'] == 'pickup' && $this->config->get($result['code'] . '_status') && $data['pickup_delivery'] ) {
                $data['pickupEnabled'] = true;
            }

            if ($result['code'] == 'pickup' && $this->config->get($result['code'] . '_status')) {
                $data['storePickupExtensionEnabled'] = true;
            }

            if ($result['code'] == 'store_delivery' && $this->config->get($result['code'] . '_status') && $data['delivery_by_owner']) {
                $data['storeDeliveryEnabled'] = true;
            }

            if ($result['code'] == 'store_delivery' && $this->config->get($result['code'] . '_status')) {
        ...
        $category_for_store = array();
        $data['store_categories_commission'] = array();
        $data['store_top_categories'] = array();
    ...
    protected function validateForm() {
        ...
        if (isset($this->request->post['commission_type']) && $this->request->post['commission_type'] != 'store' && count($this->request->post['category_commission']) > 0 ) {
