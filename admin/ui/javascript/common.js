...
$(document).ready(function() {
	...
	$(document).delegate('button[data-toggle=\'image\']', 'click', function() {
		...
		$.ajax({
			url: 'index.php?path=common/filemanager&token=' + getURLVar('token'),
			dataType: 'html',
			beforeSend: function() {
	...
	$(document).delegate('a[data-toggle=\'image\']', 'click', function(e) {
		...
		$('#button-image').on('click', function() {
			...
			$.ajax({
				url: 'index.php?path=common/filemanager&token=' + getURLVar('token') + '&target=' + $(element).parent().find('input').attr('id') + '&thumb=' + $(element).attr('id'),
				dataType: 'html',
				beforeSend: function() {
					$('#button-image i').replaceWith('<i class="fa fa-circle-o-notch fa-spin"></i>');
					$('#button-image').prop('disabled', true);
    ...
    var skip_paths = ['', undefined, 'common/login', 'common/dashboard', 'appearance/layout', 'module/brainyfilter', 'localisation/order_status/edit'];

    if ($.inArray(path, skip_paths) === -1) {
