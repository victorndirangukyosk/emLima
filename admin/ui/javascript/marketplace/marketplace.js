...
Marketplace.installfromweb = function(install_url, product_id, product_name, product_version, obj) {
	...
	$.ajax({
		url: 'index.php?path=extension/installer/install&token=' + token,
		dataType: 'json',
		type: 'post',
...
Marketplace.apps.initialize = function() {
	...
	Marketplace.loadweb(baseUrl+'index.php?path=extension/marketplace/api&api=api/marketplace');
};

Marketplace.cart = {
	'add': function(product_id, quantity) {
		$.ajax({
			url: baseUrl+'index.php?path=extension/marketplace/api&api=api/cart/add&version='+apps_version+'&token='+token+'&store='+store,
			type: 'post',
...
$(document).on('keypress', '#marketplace-search-input', function() {
		...
		if(keycode == '13'){
			...
			Marketplace.loadweb( baseUrl + 'index.php?path=extension/marketplace/api&api=api/search&search=' + search + '&type=' + searchFilter + store_name );
		}
});
...
$(document).on('click', '.uninstall-button', function(e) {
	$.ajax({
		url: 'index.php?path=extension/installer/uninstall&product_id=' + $(this).attr('data-product-id') + '&token=' + token,
		dataType: 'json',
		beforeSend: function() {
