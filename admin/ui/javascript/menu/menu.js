$( document ).ready(function() {
	...
	$('input[name=\'filter_category_name\']').autocomplete({
		'source': function(request, response) {
			$.ajax({
				url: 'index.php?path=appearance/menu/autocomplete&token=' + token + '&filter_category_name=' +  encodeURIComponent(request),
				dataType: 'json',
	...
	$('input[name=\'filter_product_name\']').autocomplete({
		'source': function(request, response) {
			$.ajax({
				url: 'index.php?path=appearance/menu/autocomplete&token=' + token + '&filter_product_name=' +  encodeURIComponent(request),
				dataType: 'json',
				success: function(json) {
					response($.map(json, function(item) {
						return {
	...
	$('input[name=\'filter_manufacturer_name\']').autocomplete({
		'source': function(request, response) {
			$.ajax({
				url: 'index.php?path=appearance/menu/autocomplete&token=' + token + '&filter_manufacturer_name=' +  encodeURIComponent(request),
				dataType: 'json',
				success: function(json) {
					response($.map(json, function(item) {
						return {
	...
	$('input[name=\'filter_information_name\']').autocomplete({
		'source': function(request, response) {
			$.ajax({
				url: 'index.php?path=appearance/menu/autocomplete&token=' + token + '&filter_information_name=' +  encodeURIComponent(request),
				dataType: 'json',
				success: function(json) {
					response($.map(json, function(item) {
						return {
...
function saveMenu(wrap_id, menu_show_id) {
...
function statusMenu(type, menu_id, menu_show_id, id) {
	var listing = menu_show_id.split('-');
		
	if(listing[1] == 'child') {
		if(type == 'enable'){
			var data_href = statusMenuChildEnable;
		} else {
			var data_href = statusMenuChildDisable;
		}
	} else {
		if(type == 'enable'){
			var data_href = statusMenuEnable;
		} else {
			var data_href = statusMenuDisable;
		}
	}
			...
			$.fn.extend({
				...
				updateParentMenuItemDBId : function() {
					return this.each(function(){
						...
						var menuType = ( parent.find( '.menu-item-data-typeMenu' ).val() );

							
						if ( 0 === depth ) { // Item is on the top level, has no parent
							input.val(0);
//							menuType.val('MainMenu');
						} else { // Find the parent item, and retrieve its object id.
							if( menuType == 'ChildMenu') {
								input.val( parent.find( '.menu-item-data-db-id' ).val() + '_1');
							} else {
		...
		initSortables : function() {
			...
			api.menuList.sortable({
				...
				stop: function(e, ui) {
					...
					$.ajax({
						url: changeMenuPosition,
						type: 'post',
						data: jQuery('#form-menu').serialize(),
						error: function(xhr, ajaxOptions, thrownError) {
							alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
						}
					});
