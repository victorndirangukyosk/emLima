var Layout = function() {
  ...
  return{
    ...
    refresh_module_list : function() {
      $.ajax({
        url : 'index.php?path=appearance/layout/getModuleList&token=' + token,
        dataType : 'html',

        success : function(textStatus) {
          $('.module_accordion').html(textStatus);
          Layout['handleAccordion']();
          Layout['handleDraggable']();
        }
      });
...
function refresh_layout() {
    $.ajax({
        url: 'index.php?path=appearance/layout/getLayoutList&token=' + token,
        type: 'post',
        dataType: 'html',
        cache: false,
        success: function(html) {
