    ...
    <div class="page-header">
        <div class="container-fluid">
            ...
            <?php }else{ ?>
                ...
                <!--<button type="button" data-toggle="tooltip" title="<?php echo $button_delete; ?>" class="btn btn-danger" onclick="confirm('<?php echo $text_confirm; ?>') ? $('#form-product').submit() : false;"><i class="fa fa-trash-o"></i></button>-->
            <?php } ?>
				<span style="margin-left: 10px;" onclick="ChangeInventoryBuyingPrice()" form="form-product" data-toggle="tooltip" title="Change Price" class="btn btn-success"><i class="fa fa-check"></i></span>
            </div>
            ...
            <div class="panel-body">
                ...
                <form action="<?php echo $delete; ?>" method="post" enctype="multipart/form-data" id="form-product">
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <td class="text-left"><?php echo $column_status; ?></a>
                                     ...
                                     <td class="text-center"><?php echo 'Buying Price'; ?></td>
				    ...
				    <td class="text-left">
                                    ...
                                    <td class="text-right"><button type="button" onclick="ChangeProductInventoryBuyingPrice('<?php echo $product['product_store_id']; ?>');" data-toggle="tooltip" title="History" class="btn btn-default" data-original-title="Save"><i class="fa fa-check-circle text-success"></i></button>
									  ...
									  <div class="modal-dialog">
										...
										<div class="modal-content">
										  <div style="color: white;background-color: #008db9;" class="modal-header">
											...
											<h4 class="modal-title"><strong>Inventory Price History : <?php echo $product['name']; ?></strong></h4>
										  </div>
...
function submit_copy() {
  ...
  $('#button-filter').on('click', function() {
            ...
            var url = 'index.php?path=catalog/vendor_product/Manageinventory&token=<?php echo $token; ?>';
...
function getProductInventoryHistory(product_store_id){
	  $('.modal-body').html('');
	   $.ajax({
                    url: 'index.php?path=catalog/product/getProductInventoryPriceHistory&token=<?= $token ?>',
                    dataType: 'html',
...
function ChangeProductInventoryBuyingPrice(product_store_id){
    var data_array = [];
    $(".buying_price").each(function() {
        var tempObj ={};
        var buying_price = $(this).val();
	    ...
	    var current_buying_price = $(this).attr('data-current-buying-price');
            var source = $('#source_'+vendor_product_id).val();
            var current_source = $('#source_'+vendor_product_id).attr('data-current-source');
            var store_id = 75;
            var added_by = <?php echo $this->user->getId() ?>;
            tempObj.product_store_id = vendor_product_id;
            tempObj.product_id = general_product_id;
            tempObj.product_name = product_name;
            tempObj.buying_price = buying_price;
            tempObj.source = source;
            tempObj.current_source = current_source;
	    tempObj.current_buying_price = current_buying_price;
            tempObj.store_id = store_id;
            tempObj.added_by = added_by;
            console.log(product_store_id);
            console.log(vendor_product_id);
    ...
    if(data_array.length  > 0){
           $.ajax({
                    url: 'index.php?path=catalog/product/updateInventoryPricing&token=<?= $token ?>',
                    dataType: 'json',
...
function ChangeInventoryBuyingPrice(){
    var data_array = [];
    $(".buying_price").each(function() {
        var tempObj ={};
        var buying_price = $(this).val();
