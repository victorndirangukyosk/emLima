            ...
            <div class="row">
                <div class="col-sm-8">
                    <div class="form-group">
                        ...
                        <input type="text" name="filter_question" value="<?php echo $filter_question; ?>" placeholder="<?php echo $column_question; ?>" id="input-question" class="form-control" autocomplete="off" />
                    </div>
                </div>
                <div class="col-sm-4">                    
        ...
        <form action="<?php echo $delete; ?>" method="post" enctype="multipart/form-data" id="form-category">
          <div class="table-responsive">
            <table class="table table-bordered table-hover">
              ...
              <tbody>
                <?php if ($helps) { ?>
                <?php foreach ($helps as $category) { ?>
                <tr>
                  <td class="text-center"><?php if (in_array($category['help_id'], $selected)) { ?>
                    <input type="checkbox" name="selected[]" value="<?php echo $category['help_id']; ?>" checked="checked" />
                    <?php } else { ?>
                    <input type="checkbox" name="selected[]" value="<?php echo $category['help_id']; ?>" />
                    <?php } ?></td>
                  <td class="text-left"><?php echo $category['question']; ?></td>
...
$('#button-filter').on('click', function() {
    url = 'index.php?path=catalog/help&token=<?php echo $token; ?>';
...
$('input[name=\'filter_question\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?path=catalog/help/autocomplete&token=<?php echo $token; ?>&filter_question=' +  encodeURIComponent(request),
			dataType: 'json',
