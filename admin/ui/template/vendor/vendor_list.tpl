        ...
        <div class="well" style="display:none;">
          <div class="row">
            <div class="col-sm-4">
              ...
              <div class="form-group">
                <label class="control-label" for="input-user-group"><?php echo $entry_vendor_group; ?></label>
                <input type="text" name="filter_user_group" value="<?php echo $filter_user_group; ?>" placeholder="<?php echo $entry_vendor_group; ?>" id="input-user-group" class="form-control" />
              </div>
              <div class="form-group">
            ...
            <table class="table table-bordered table-hover">
              <thead>
                <tr>
                 ...
                 <td class="text-left"><?php if ($sort == 'c.name') { ?>
                    <a href="<?php echo $sort_city; ?>" class="<?php echo strtolower($order); ?>"><?= $column_city ?></a>
                    <?php } else { ?>
                    <a href="<?php echo $sort_city; ?>"><?= $column_city ?></a>
                    <?php } ?>
                 </td>
                 <td class="text-left"><?php if ($sort == 'u.status') { ?>
                    <a href="<?php echo $sort_status; ?>" class="<?php echo strtolower($order); ?>"><?php echo $column_status; ?></a>
                    <?php } else { ?>
                    <a href="<?php echo $sort_status; ?>"><?php echo $column_status; ?></a>
                    <?php } ?></td>
                  <td class="text-left"><?php if ($sort == 'u.date_added') { ?>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td class="text-left"><?php echo $user['city']; ?></td>
                  <td class="text-left"><?php echo $user['status']; ?></td>
                  <td class="text-left"><?php echo $user['date_added']; ?></td>
                  <td class="text-right">
                     <a href="<?php echo $user['login']; ?>" data-toggle="tooltip" title="Login As vendor" class="btn btn-primary"><i class="fa fa-key"></i></a>
                     
                      <a href="<?php echo $user['info']; ?>" data-toggle="tooltip" title="Info" class="btn btn-info"><i class="fa fa-info"></i></a>
...
$('#button-filter').on('click', function() {
	var url = 'index.php?path=vendor/vendor&token=<?php echo $token; ?>';
...
$('input[name=\'filter_city\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?path=vendor/vendor/city_autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request),
			dataType: 'json',
	...
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?path=vendor/vendor/autocomplete&token=<?php echo $token; ?>&filter_user_name=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
					return {
...
$('input[name=\'filter_user_group\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?path=vendor/vendor/autocomplete&token=<?php echo $token; ?>&filter_user_group=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
					return {
						label: item['vendor_group'],
...
$('input[name=\'filter_first_name\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?path=vendor/vendor/autocomplete&token=<?php echo $token; ?>&filter_first_name=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
					return {
...
$('input[name=\'filter_last_name\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?path=vendor/vendor/autocomplete&token=<?php echo $token; ?>&filter_last_name=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
					return {
...
$('input[name=\'filter_email\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?path=vendor/vendor/autocomplete&token=<?php echo $token; ?>&filter_email=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
					return {
