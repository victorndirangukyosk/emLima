...
class ControllerExtensionPaymentIugu extends Controller
{
    ...
    {
        $data = $this->load->language('extension/payment/iugu');
        ...
        $data['breadcrumbs'][] = [
            'href' => $this->url->link('extension/payment/iugu'),
        ...
        $data['action'] = $this->url->link('extension/payment/iugu', 'token='.$this->session->data['token'], 'SSL');
        ...
        $this->response->setOutput($this->load->view('extension/payment/iugu.tpl', $data));
    }

    protected function validate()
    {
        if (!$this->user->hasPermission('modify', 'extension/payment/iugu')) {
