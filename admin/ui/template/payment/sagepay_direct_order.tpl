...
<table class="table table-striped table-bordered">
  ...
  <tr>
    ...
    <td id="sagepay_direct_total_released"><?php echo $sagepay_direct_order['total_released_formatted']; ?></td>
  </tr>
  <tr>
    <td><?php echo $text_release_status; ?></td>
    <td id="release_status"><?php if ($sagepay_direct_order['release_status'] == 1) { ?>
      ...
      <input type="text" width="10" id="release_amount" value="<?php echo $sagepay_direct_order['total']; ?>"/>
  ...
  <tr>
    ...
    <td id="void_status"><?php if ($sagepay_direct_order['void_status'] == 1) { ?>
  ...
  <tr>
    ...
    <td id="rebate_status"><?php if ($sagepay_direct_order['rebate_status'] == 1) { ?>
  ...
  $("#button-void").click(function() {
		if (confirm('<?php echo $text_confirm_void; ?>')) {
			$.ajax({
				...
				url: 'index.php?path=payment/sagepay_direct/void&token=<?php echo $token; ?>',
				beforeSend: function() {
					$('#button-void').hide();
	...
	$("#button-release").click(function() {
		if (confirm('<?php echo $text_confirm_release; ?>')) {
			$.ajax({
				...
				url: 'index.php?path=payment/sagepay_direct/release&token=<?php echo $token; ?>',
				beforeSend: function() {
					$('#button-release').hide();
					$('#release_amount').hide();
	...
	$("#button-rebate").click(function() {
		if (confirm('<?php echo $text_confirm_rebate ?>')) {
			$.ajax({
				...
				url: 'index.php?path=payment/sagepay_direct/rebate&token=<?php echo $token; ?>',
				beforeSend: function() {
					$('#button-rebate').hide();
					$('#rebate_amount').hide();
