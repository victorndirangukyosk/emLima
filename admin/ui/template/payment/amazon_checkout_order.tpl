...
$('#button-upload').on('click', function() {
	...
	$('#form-upload input[name=\'file\']').on('change', function() {
		$.ajax({
			url: 'index.php?path=payment/amazon_checkout/uploadOrderAdjustment&token=<?php echo $token; ?>',
			type: 'post',
			dataType: 'json',
			...
			success: function(json) {
				...
				if (json['success']) {
					...
					$.get('index.php?path=payment/amazon_checkout/addSubmission&order_id=<?php echo $order_id ?>&submission_id=' + json['submission_id'] + '&token=<?php echo $token; ?>');
				}
