		...
		<div class="well" style="display:none;">
          <div class="row">
            <div class="col-sm-12">
              ...
              <div class="form-group">
                <label class="control-label" for="input-filter-description"><?php echo $column_description; ?></label>
                <input type="text" name="filter_description" value="<?php echo $filter_description; ?>" placeholder="<?php echo $column_description; ?>" id="input-filter-description" class="form-control" />
              </div>
...
$('#button-filter').on('click', function() {
	var url = 'index.php?path=localisation/geo_zone&token=<?php echo $token; ?>';
...
$('input[name=\'filter_name\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?path=localisation/geo_zone/autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request),
			dataType: 'json',
...
$('input[name=\'filter_description\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?path=localisation/geo_zone/autocomplete&token=<?php echo $token; ?>&filter_description=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
					return {
