...
<div id="content">
                                    ...
                                    <tr>
                                        <td class="text-left">
                                            ...
                                            <div id="">
                                                ...
                                                <img src="ui/image/flags/<?php echo $language['image']; ?>" title="<?php echo $language['name']; ?>" align="top" />
                                            </div>
                                            <?php } ?>

                                        </td>
                                        <td class="text-left"><button type="button" onclick="$(this).parents('tr').remove();" data-toggle="tooltip" title="<?php echo $button_remove; ?>" class="btn btn-danger"><i class="fa fa-minus-circle"></i></button>
                                        </td>
                                    </tr>
                                </tbody>
                                <?php $module_row++; ?>
    ...
    $('input[name=\'author_name\']').autocomplete({
        'source': function(request, response) {
            $.ajax({
                url: 'index.php?path=simple_blog/author/autocomplete&token=<?php echo $token; ?>&author_name=' + encodeURIComponent(request),
                    dataType: 'json',
    ...
    <script type="text/javascript">
        ...
        function addDescription() {
            ...
            <?php foreach ($languages as $language) { ?>
            html += '<div id="">'
            html += '	<textarea name="article_addition_description[' + module_row + '][<?php echo $language['language_id']; ?>][additional]" id="description-' + module_row + '-<?php echo $language['language_id']; ?>" class="form-control"></textarea> <img src="ui/image/flags/<?php echo $language['image']; ?>" title="<?php echo $language['name']; ?>" align="top" />';
            html += '</div>';
            <?php } ?>
            html += '           ';
            html += '       </td>';
...
<script type="text/javascript">
    ...
    'source': function(request, response) {
    $.ajax({
    url: 'index.php?path=simple_blog/article/autocomplete_article&token=<?php echo $token; ?>&simple_blog_article_id=<?php echo $simple_blog_article_id; ?>&filter_name=' + encodeURIComponent(request),
            dataType: 'json',
            success: function(json) {
            response($.map(json, function(item) {
            return {
