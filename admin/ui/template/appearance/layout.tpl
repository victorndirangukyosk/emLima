              ...
              <div class="col-md-3 col-sm-4 hidden-xs pull-left" style="padding-left: 40px;">
				...
				<div class="layout">
				  ...
				  <div class="col-sm-5 layout-buttons">
					  ...
					  <button type="button" data-toggle="tooltip" title="" class="btn btn-danger" onclick="confirm('Are you sure?') ? layoutDelete('<?php echo $layout_id; ?>') : false;" data-original-title="Layout Delete"><i class="fa fa-trash-o"></i></button>
				  </div>
				 </div>
				 </div>
				<div class="row">
                    ...
                    <div id="module_list" class="module_list" data-text-confirm="<?php echo $text_confirm;?>" data-text-edit="<?php echo $text_edit;?>">
                        ...
                        <div class="module_accordion accordion">
                            <?php foreach($extensions as $modules) { ?>
								  ...
								  <?php if(!empty($modules['instance'])) { ?>
								  	<?php if(!empty($modules['module'])) { ?>
										...
										<?php foreach($modules['module'] as $module) { ?>
											<div class="module-block ui-draggable" data-code="<?php echo $module['code']; ?>" id="<?php echo str_replace('.', '_', $module['code']); ?>"><i class="fa fa-arrows-alt"></i><span class="module-name"><?php echo $module['name']; ?></span>
								  ...
								  <?php } else { ?>
								  <div class="accordion-content accordion-content-drag">
									  <div class="module-block ui-draggable" data-code="<?php echo $modules['code']; ?>"><i class="fa fa-arrows-alt"></i><span class="module-name"><?php echo $modules['name']; ?></span>
...
$(document).ready(function() {	
		...
		$("#change_layouts").change(function() {
		  window.location.href='<?php echo str_replace("&amp;","&",$refresh); ?>&layout_id=' + $(this).val();
		});
...
function removeModule(module_id, id){
	$.ajax({
		url: 'index.php?path=appearance/layout/removeModule&token=<?php echo $token;?>',
		type: 'post',
...
function removeLayoutModule(layout_id, layout_module_id, module){
	$.ajax({
		url: 'index.php?path=appearance/layout/removeLayoutModule&token=<?php echo $token;?>',
		type: 'post',
...
function layoutDelete(layout_id){
    var layoutID = layout_id;
	$.ajax({
		url: 'index.php?path=appearance/layout/removeLayout&token=<?php echo $token;?>',
		type: 'post',
		data: {layout_id: layout_id},
		dataType: 'json',
		cache: false,
		success: function(json) {
			 window.location.href='<?php echo str_replace("&amp;","&",$refresh); ?>&layout_id=' + layoutID;
		},
		error: function(xhr, ajaxOptions, thrownError) {
			alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
		}
...
var action_url = '<?php echo str_replace("amp;", "", $action); ?>';
    //--></script>
