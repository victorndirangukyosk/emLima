        ...
        <div class="well" style="display:none;">
          <div class="row">
            <div class="col-sm-4">
              ...
              <div class="form-group">
                <label class="control-label" for="input-user-group"><?= $entry_city ?></label>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td class="text-right">
                      ...
                      <a href="<?php echo $user['today']; ?>" data-toggle="tooltip" title="Today report" class="btn btn-info">
                          <i class="fa fa-truck"></i>
                      </a>
                      
                      <a href="<?php echo $user['timeslots']; ?>" data-toggle="tooltip" title="Timeslots" class="btn btn-info"><i class="fa fa-clock-o"></i></a>
...
$('#button-filter').on('click', function() {
	var url = 'index.php?path=shopper/shopper&token=<?php echo $token; ?>';
...
$('input[name=\'filter_user_name\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?path=shopper/shopper/autocomplete&token=<?php echo $token; ?>&filter_user_name=' +  encodeURIComponent(request),
			dataType: 'json',
...
$('input[name=\'filter_user_group\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?path=shopper/shopper/autocomplete&token=<?php echo $token; ?>&filter_user_group=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
					return {
						label: item['shopper_group'],
...
$('input[name=\'filter_first_name\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?path=shopper/shopper/autocomplete&token=<?php echo $token; ?>&filter_first_name=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
					return {
...
$('input[name=\'filter_last_name\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?path=shopper/shopper/autocomplete&token=<?php echo $token; ?>&filter_last_name=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
					return {
...
$('input[name=\'filter_city\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?path=shopper/shopper/city_autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
					return {
...
$('input[name=\'filter_email\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?path=shopper/shopper/autocomplete&token=<?php echo $token; ?>&filter_email=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
					return {
