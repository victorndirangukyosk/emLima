<?php  echo $header;  ?>
	  ...
	  <?php } ?>
		<div class="box">
		   ...
		   <div class="toolbar"><?php require( dirname(__FILE__).'/toolbar.tpl' ); ?></div>
			...
			<div class="heading">
			  <h1><img src="ui/image/module.png" alt="" /> <?php echo $heading_title; ?></h1>
				<div class="buttons">
					...
					<form id="form" enctype="multipart/form-data" method="post" action="<?php echo $action;?>">
							...
							<div id="tab-general">
									...
									<div id="language-blog" class="htabs pavtabs">
										...
										<a href="#tab-language-<?php echo $language['language_id']; ?>"><img src="ui/image/flags/<?php echo $language['image']; ?>" title="<?php echo $language['name']; ?>" /> <?php echo $language['name']; ?></a>
										<?php } ?>
									  </div>
									  <?php foreach ($languages as $language) { ?>
 ...
 <script type="text/javascript" src="ui/javascript/jquery/ui/jquery-ui-timepicker-addon.js"></script> 
...
function image_upload(field, thumb) {
	...
	$('#content').prepend('<div id="dialog" style="padding: 3px 0px 0px 0px;"><iframe src="index.php?path=common/filemanager&token=<?php echo $token; ?>&field=' + encodeURIComponent(field) + '" style="padding:0; margin: 0; display: block; width: 100%; height: 100%;" frameborder="no" scrolling="auto"></iframe></div>');
	...
	$('#dialog').dialog({
		...
		close: function (event, ui) {
			if ($('#' + field).attr('value')) {
				$.ajax({
					url: 'index.php?path=common/filemanager/image&token=<?php echo $token; ?>&image=' + encodeURIComponent($('#' + field).attr('value')),
					dataType: 'text',
