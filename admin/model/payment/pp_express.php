...
class ModelPaymentPPExpress extends Model
{
    ...
    public function updatePaypalOrder($status, $parent_transaction_id)
    {
        $this->db->query('UPDATE `'.DB_PREFIX.'paypal_order_transaction` SET `payment_status` = '.$status." WHERE `transaction_id` = '".$parent_transaction_id."' LIMIT 1");
    }

    public function multipleUpdate($recurring_id)
    {
        $this->db->query('INSERT INTO `'.DB_PREFIX."order_recurring_transaction` SET `order_recurring_id` = '".$recurring_id."', `date_added` = NOW(), `type` = '5'");
        $this->db->query('UPDATE `'.DB_PREFIX."order_recurring` SET `status` = 4 WHERE `order_recurring_id` = '".$recurring_id."' LIMIT 1");
    }
}
