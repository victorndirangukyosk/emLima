...
class ModelPaymentSecureTradingPp extends Model
{
    ...
    public function void($order_id)
    {
        ...
        if (!empty($securetrading_pp_order) && 0 == $securetrading_pp_order['release_status']) {
    ...
    public function release($order_id, $amount)
    {
        ...
        if (!empty($securetrading_pp_order) && 0 == $securetrading_pp_order['release_status'] && $total_released <= $amount) {
            $requestblock_xml = new SimpleXMLElement('<requestblock></requestblock>');
            $requestblock_xml->addAttribute('version', '3.67');
            $requestblock_xml->addChild('alias', $this->config->get('securetrading_pp_webservice_username'));
    ...
    public function rebate($order_id, $refunded_amount)
    {
        ...
        if (!empty($securetrading_pp_order) && 1 != $securetrading_pp_order['rebate_status']) {
            $requestblock_xml = new SimpleXMLElement('<requestblock></requestblock>');
            $requestblock_xml->addAttribute('version', '3.67');
            $requestblock_xml->addChild('alias', $this->config->get('securetrading_pp_webservice_username'));
