...
function error_handler_for_export_import( $errno, $errstr, $errfile, $errline ) {
	...
	if ( ( $errors != "Fatal Error" ) && isset( $request->get['path'] ) && ( $request->get['path'] != 'tool/export_import/download' ) ) {
...
class ModelToolExportImport extends Model {
	...
	protected function validateProducts( &$reader ) {
		...
		$expected_heading = array("product_id", "product_sr_no", "name","default_price","unit","weight","variations_id", "categories" );

		$expected_heading = array_merge( $expected_heading, array( "model", "image_name", "default_variation_name", "date_added", "date_modified", "status", "seo_keyword", "description" ) );
		if ( $exist_meta_title ) {
			$expected_heading[] = "meta_title";
		}
		$expected_heading = array_merge( $expected_heading, array( "meta_description", "meta_keywords", "tags", "sort_order" ) );
		if ( $exist_meta_title ) {
	...
	protected function validateProductVariations( &$reader ) {
		...
		$expected_heading = array( "id", "product_id", "product_sr_no", "name", "image", "model", "sort_order" );
