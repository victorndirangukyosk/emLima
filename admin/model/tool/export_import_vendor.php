...
class ModelToolExportImportVendor extends Model {
	...
	function validateHeading( &$data, &$expected, &$multilingual = array() ) {
	...
	function populateVendorProductsWorksheet( &$worksheet, &$languages, $default_language_id, &$price_format, &$box_format, &$weight_format, &$text_format, $offset = null, $rows = null, &$min_id = null, &$max_id = null ) {
	...
	function populateSampleProductsWorksheet( &$worksheet, &$languages, $default_language_id, &$price_format, &$box_format, &$weight_format, &$text_format, $offset = null, $rows = null, &$min_id = null, &$max_id = null ) {
	...
	function populateModelsNotPresentWorksheet( &$worksheet, &$languages, $default_language_id, &$price_format, &$box_format, &$weight_format, &$text_format, $offset = null, $rows = null, &$min_id = null, &$max_id = null ,$model_not_found = []) {
	...
	*/
	...
	protected function validateProductIdColumns( &$reader ) {
		...
		$worksheets = array( 'ProductVariations' );
	...
	protected function validateProducts( &$reader ) {
		...
		$expected_heading = array( "barcode", "item name","department","sell price","pricedropsp","startdate","enddate","purchase block");
...
(
	...
	protected function validateProductVariations( &$reader ) {
		...
		$expected_heading = array( "id", "product_store_sr_id", "variation_id", "variation_name", "model" , "price", "special_price");
	...
	public function uploadProducts(&$reader, $incremental, &$available_product_ids = array() ){
    ...
    protected function storeProductIntoDatabaseOld( &$product,$incremental) {
        ...
        if($exists && $incremental) {
        	...
        	$data['status'] = ( ( strtoupper( $status ) == "TRUE" ) || ( strtoupper( $status ) == "YES" ) || ( strtoupper( $status ) == "ENABLED" ) ) ? 1 : 0;
    ...
    protected function storeProductIntoDatabase(&$product,$incremental) {
        ...
        if($exists && $incremental) {
        	...
        	//echo "<pre>";print_r("in if nicremen");die;
        	$subtract_quantity = strlen($product['subtract_quantity'])?$product['subtract_quantity']: "false";
        	$data['status'] = ( ( strtoupper( $status ) == "TRUE" ) || ( strtoupper( $status ) == "YES" ) || ( strtoupper( $status ) == "ENABLED" ) || empty($status) || $status ) ? 1 : 0;
        	$data['subtract_quantity'] = ( ( strtoupper( $subtract_quantity ) == "TRUE" ) || ( strtoupper( $subtract_quantity ) == "YES" ) || ( strtoupper( $subtract_quantity ) == "ENABLED" ) ) ? 1 : 0;
    ...
    protected function uploadProductVariations( &$reader, $incremental, &$available_category_ids = array() ) {
